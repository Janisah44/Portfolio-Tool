    ...
    if fund_cf_data:
        ...
        for cf_deal in fund_cf_data:
            if cf_deal['name'] == deal['name']:
                # Get latest month's NAV
                if cf_deal.get('nav', 0) > 0:
                    nav = cf_deal['nav']
        ...
        if is_csv:
            ...
            if header_row:
                ...
                for row in range(header_row + 1, min(header_row + 100, ws.max_row + 1)):
                    ...
                    # Find NAV column (should be around column 537+)
                    nav_col_start = headers.get('NAV') or 537

                    # Get latest NAV value (scan backwards from last column)
                    for col in range(min(ws.max_column, nav_col_start + 200), nav_col_start - 1, -1):
                        try:
                            val = ws.cell(row, col).value
                            if val and isinstance(val, (int, float)) and val != 0:
                                nav = float(val) / 1000000  # Convert to millions
                                break
                        except:
                            continue

                    # If no NAV found, use commitment
                    if nav == 0:
                        nav = commitment
                    ...
                    # Extract monthly cashflows - columns 14+ (skip for now, focus on NAV)
                    # for col in range(14, min(ws.max_column + 1, 190)):
                    #     try:
