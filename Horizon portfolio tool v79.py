...
def route(tab, portfolio, pipeline, placeholders, config, cf_data, liq_data, disc_store):
    ...
    if tab == "analytics":  return _safe(tab_analytics, p, pi, config)
...
def tab_proforma(portfolio, pipeline, config):
    ...
    # Combined table: current deals + pipeline
    total = all_nav or 1
    rows = []
    for d in portfolio:
        rows.append({
    ...
    return html.Div([
        html.Div([
            ...
            html.Span("  (read-only — manage deals on Portfolio and Pipeline tabs)",
...
def tab_deploy(placeholders, config):
    ...
    fig.update_layout(**CHART, height=300, title="Planned Commitment Schedule ($M)")
...
def tab_cashflows(portfolio, cf_data, disc_store):
    ...
    if disc_rows:
        ...
        fig_disc = go.Figure()
        fig_disc.update_layout(**CHART, height=200)
...
def tab_liquidity(liq_data):
    ...
    if nav_proj:
        ...
        fig_nav.update_layout(**CHART, height=340, title="NAV End Projections",
                               yaxis_title="NAV ($M)", hovermode="x unified")
    else:
        fig_nav = go.Figure()
        fig_nav.update_layout(**CHART, height=200, title="NAV Projections (no data)")
...
def add_pipeline(_, pipeline, nid, edit_idx, name, strategy, stage_deal, sector,
    ...
    deal = dict(name=name, strategy=strategy, stage_deal=stage_deal,
    ...
    if edit_idx is not None and 0 <= edit_idx < len(pipeline):
        deal["id"] = pipeline[edit_idx]["id"]
        pipeline[edit_idx] = deal
        return pipeline, nid, f"✓ Updated pipeline: {name}"
    deal["id"] = nid
    return pipeline+[deal], nid+1, f"✓ Added pipeline: {name}"
...
def populate_pipe_edit(_, selected, pipeline):
    ...
    if idx >= len(pipeline):
        return [no_update]*10
    d = pipeline[idx]
    return (idx, d.get("name",""), d.get("strategy", STRATEGIES[0]), d.get("stage_deal","Buyout"),
            ...
            d.get("priority","Medium"), f"📝 Editing pipeline deal: {d.get('name','')} — click '+ Add / Save' to update.")
...
def add_placeholder(_, placeholders, nid, edit_idx, name, strategy, dtype, region, size, irr, month):
    if not name:
        return no_update, no_update, "⚠ Enter a name."
    placeholders = list(placeholders or [])
    ph = dict(name=name, strategy=strategy, deal_type=dtype,
    ...
    if edit_idx is not None and 0 <= edit_idx < len(placeholders):
        ph["id"] = placeholders[edit_idx]["id"]
        placeholders[edit_idx] = ph
        return placeholders, nid, f"✓ Updated: {name}"
    ph["id"] = nid
    return placeholders+[ph], nid+1, f"✓ Added: {name}"
...
def populate_ph_edit(_, selected, placeholders):
    ...
    if idx >= len(placeholders):
        return [no_update]*8
    d = placeholders[idx]
    return (idx, d.get("name",""), d.get("strategy", STRATEGIES[0]), d.get("deal_type","Secondary"),
            d.get("region","North America"), d.get("size",0), round(float(d.get("target_irr",0))*100,2),
            d.get("expected_month",0), f"📝 Editing future deployment: {d.get('name','')} — click '+ Add / Save' to update.")
...
# ── Pro forma: save inline edits back to portfolio / pipeline ───────────────
@app.callback(
    Output("port-store","data", allow_duplicate=True),
    Output("pipe-store","data", allow_duplicate=True),
...
def save_proforma_inline(_, rows, portfolio, pipeline):
    if not rows:
    ...
    def parse_money(v):
        ...
        if isinstance(v, (int, float)):
            return float(v)
        s = str(v).replace("$", "").replace(",", "").replace("M", "").strip()
    ...
    for row in rows:
        ...
        if source == "portfolio" and port_i < len(portfolio):
            d = dict(portfolio[port_i])
            d["name"] = row.get("Deal", d.get("name",""))
            d["manager"] = row.get("Manager", d.get("manager",""))
            d["strategy"] = row.get("Strategy", d.get("strategy",""))
            dtype = row.get("Type", d.get("deal_type",""))
            if dtype in DEAL_TYPES:
                d["deal_type"] = dtype
            region = row.get("Region", d.get("region",""))
            if region:
                d["region"] = region
            nav = parse_money(row.get("NAV"))
            if nav is not None:
                d["nav"] = nav
            irr = parse_pct(row.get("IRR %"))
            if irr is not None:
                d["target_irr"] = irr
            moic = parse_moic(row.get("MOIC"))
            if moic is not None:
                d["moic"] = moic
            try:
                d["vintage"] = int(str(row.get("Vintage", d.get("vintage", datetime.now().year))).replace("—", "").strip() or d.get("vintage", datetime.now().year))
            except Exception:
                pass
            d["allocation_status"] = row.get("Status", d.get("allocation_status",""))
            portfolio[port_i] = d
            changed_port = True
            port_i += 1
        elif source == "pipeline" and pipe_i < len(pipeline):
            d = dict(pipeline[pipe_i])
            d["name"] = row.get("Deal", d.get("name",""))
            d["strategy"] = row.get("Strategy", d.get("strategy",""))
            region = row.get("Region", d.get("region",""))
            if region:
                d["region"] = region
            size = parse_money(row.get("NAV"))
            if size is not None:
                d["size"] = size
            irr = parse_pct(row.get("IRR %"))
            if irr is not None:
                d["target_irr"] = irr
            status = row.get("Status", d.get("pipeline_stage",""))
            if status:
                d["pipeline_stage"] = status
            pipeline[pipe_i] = d
            changed_pipe = True
            pipe_i += 1
    return (portfolio if changed_port else no_update, pipeline if changed_pipe else no_update)
