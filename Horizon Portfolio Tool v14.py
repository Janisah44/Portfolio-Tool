...
def liquidity_assumptions_page():
    return html.Div([
        html.H2("💧 Liquidity Assumptions", className="mb-4",
                style={'fontWeight': 'bold', 'fontFamily': C['sans'], 'color': C['text']}),

        dbc.Alert([
            html.I(className="fas fa-info-circle me-2"),
            "Configure distribution rates and capital call patterns by strategy. These drive cashflow forecasts."
        ], color="info", className="mb-4"),

        # Fund-level assumptions
        dbc.Card([
            dbc.CardHeader("Fund-Level Liquidity Settings",
                           style={'fontWeight': 'bold', 'backgroundColor': C['surface']}),
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        dbc.Label("💰 Liquidity Reserve (%)", style={'fontWeight': 'bold'}),
                        dbc.Input(id="liq-reserve-pct", type="number", value=5, step=0.5,
                                  style={'fontFamily': C['mono']}),
                        html.Small("% of NAV held in cash", className="text-muted")
                    ], width=4),
                    dbc.Col([
                        dbc.Label("💵 Cash Yield (%)", style={'fontWeight': 'bold'}),
                        dbc.Input(id="liq-cash-yield", type="number", value=3, step=0.5,
                                  style={'fontFamily': C['mono']}),
                        html.Small("Annual return on cash reserves", className="text-muted")
                    ], width=4),
                    dbc.Col([
                        dbc.Label("🔄 Distribution Frequency", style={'fontWeight': 'bold'}),
                        dbc.Select(id="liq-dist-freq", options=[
                            {"label": "Monthly", "value": "monthly"},
                            {"label": "Quarterly", "value": "quarterly"},
                            {"label": "Semi-Annual", "value": "semiannual"},
                        ], value="quarterly", style={'fontFamily': C['mono']}),
                        html.Small("How often distributions occur", className="text-muted")
                    ], width=4),
                ], className="mb-3"),
            ])
        ], className="shadow-sm mb-4"),
        ...
        dbc.Card([
            dbc.CardHeader("Strategy-Level Assumptions", style={'fontWeight': 'bold', 'backgroundColor': C['surface']}),
            dbc.CardBody([
                html.Div(id='liq-strategy-assumptions')
            ])
        ], className="shadow-sm mb-4"),

        # Capital call patterns
        dbc.Card([
            dbc.CardHeader("Capital Call Patterns", style={'fontWeight': 'bold', 'backgroundColor': C['surface']}),
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        html.H6("GP-Led (Multi-Asset)", style={'color': C['blue'], 'fontFamily': C['sans']}),
                        html.P([
                            html.Strong("Pattern: "), "90% at close, 10% over 6 months",
                            html.Br(),
                            html.Strong("Draw Period: "), "0.5 years"
                        ], style={'fontSize': '13px', 'fontFamily': C['mono']})
                    ], width=3),
                    dbc.Col([
                        html.H6("GP-Led (Single-Asset)", style={'color': C['purple'], 'fontFamily': C['sans']}),
                        html.P([
                            html.Strong("Pattern: "), "80% at close, 20% over 12 months",
                            html.Br(),
                            html.Strong("Draw Period: "), "1.0 years"
                        ], style={'fontSize': '13px', 'fontFamily': C['mono']})
                    ], width=3),
                    dbc.Col([
                        html.H6("Diversified LP-Led", style={'color': C['teal'], 'fontFamily': C['sans']}),
                        html.P([
                            html.Strong("Pattern: "), "70% at close, 30% over 12 months",
                            html.Br(),
                            html.Strong("Draw Period: "), "1.0 years"
                        ], style={'fontSize': '13px', 'fontFamily': C['mono']})
                    ], width=3),
                    dbc.Col([
                        html.H6("Co-Investments", style={'color': C['green'], 'fontFamily': C['sans']}),
                        html.P([
                            html.Strong("Pattern: "), "50% at close, 50% over 24 months",
                            html.Br(),
                            html.Strong("Draw Period: "), "2.0 years"
                        ], style={'fontSize': '13px', 'fontFamily': C['mono']})
                    ], width=3),
                ])
            ])
        ...
        # Summary
        dbc.Card([
            dbc.CardHeader("Liquidity Forecast Summary", style={'fontWeight': 'bold', 'backgroundColor': C['surface']}),
            dbc.CardBody([
                html.Div(id='liq-summary')
            ])
        ], className="shadow-sm")
    ])
