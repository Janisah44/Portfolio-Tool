    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col(dbc.Card([dbc.CardBody([
                ...
                html.Small("Latest Month-End NAV (from Deal Cashflows)", className="text-muted")
            ])], className="shadow-sm"), width=2),
            dbc.Col(dbc.Card([dbc.CardBody([
                html.H6("📋 Total Commitment", className="text-muted"),
                html.H4(id="port-total-commitment", style={'color': C['blue']}),
                html.Small("Sum of All Deal Commitments", className="text-muted")
            ])], className="shadow-sm"), width=2),
            dbc.Col(dbc.Card([dbc.CardBody([
                html.H6("💵 Dry Powder", className="text-muted"),
                html.H4(id="port-dry-powder", style={'color': COLORS['primary']}),
                html.Small("Dec 2026 Liquidity Surplus", className="text-muted")
            ])], className="shadow-sm"), width=2),
            dbc.Col(dbc.Card([dbc.CardBody([
...
def future_deals_page():
    return html.Div([
        ...
        dbc.Modal([
            ...
            dbc.ModalBody([
                dcc.Store(id='edit-ph-index-store', data=None),
                dbc.Row([
                    dbc.Col([dbc.Label("Deal Name"), dbc.Input(id="edit-ph-name")], width=6),
                    dbc.Col([dbc.Label("Strategy"), dbc.Select(id="edit-ph-strategy", options=[
                        {"label": "GP-Led (Multi-Asset)", "value": "GP-Led (Multi-Asset)"},
                        {"label": "GP-Led (Single-Asset)", "value": "GP-Led (Single-Asset)"},
                        {"label": "Diversified LP-Led", "value": "Diversified LP-Led"},
                        {"label": "Co-Investments", "value": "Co-Investments"},
                ...
                dbc.Row([
                    dbc.Col([dbc.Label("Estimated Size ($mm)"), dbc.Input(id="edit-ph-size", type="number", step=0.1)],
                            width=6),
                    dbc.Col([dbc.Label("Target IRR"),
                             dbc.Input(id="edit-ph-irr", type="number", step=0.01, placeholder="0.20")], width=6),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([dbc.Label("Expected Month"),
                             dbc.Select(id="edit-ph-month", options=generate_month_options(), value=0)], width=6),
                    dbc.Col([dbc.Label("Segment"), dbc.Select(id="edit-ph-segment", options=[
                        {"label": "Seed", "value": "Seed"}, {"label": "Growth", "value": "Growth"},
                        {"label": "Pipeline", "value": "Pipeline"}
                    ])], width=6),
                ], className="mb-3"),
            ]),
            dbc.ModalFooter([
                dbc.Button("Cancel", id="btn-cancel-edit-ph", color="secondary", className="me-2"),
                dbc.Button("Save Changes", id="btn-save-edit-ph", color="success")
            ])
...
def cashflows_page():
    return html.Div([
        ...
        # Main Cashflow Tables - 4 separate tables like template: Net CF, Capital Calls, Distributions, NAV
        dbc.Card([
            dbc.CardHeader(html.Div([
                html.Span("Net Cashflows (Capital Calls + Distributions)", style={'fontWeight': 'bold'}),
                html.Small(" — editable per investment line", className="text-muted ms-2")
            ]), style={'backgroundColor': C['surface']}),
            dbc.CardBody([html.Div(id='cf-netcf-table', style={'overflowX': 'auto'})])
        ], className="shadow-sm mb-3"),

        dbc.Card([
            dbc.CardHeader(html.Div([
                html.Span("Capital Calls", style={'fontWeight': 'bold'}),
                html.Small(" — editable per investment line", className="text-muted ms-2")
            ]), style={'backgroundColor': C['surface']}),
            dbc.CardBody([html.Div(id='cf-calls-table', style={'overflowX': 'auto'})])
        ], className="shadow-sm mb-3"),

        dbc.Card([
            dbc.CardHeader(html.Div([
                html.Span("Distributions", style={'fontWeight': 'bold'}),
                html.Small(" — editable per investment line", className="text-muted ms-2")
            ]), style={'backgroundColor': C['surface']}),
            dbc.CardBody([html.Div(id='cf-dists-table', style={'overflowX': 'auto'})])
        ], className="shadow-sm mb-3"),

        dbc.Card([
            dbc.CardHeader(html.Div([
                html.Span("NAV", style={'fontWeight': 'bold'}),
                html.Small(" — editable per investment line", className="text-muted ms-2")
            ]), style={'backgroundColor': C['surface']}),
            dbc.CardBody([html.Div(id='cf-nav-table', style={'overflowX': 'auto'})])
        ], className="shadow-sm mb-4"),
        ...
        # Hidden store for uploaded data and edited cf data
        dcc.Store(id='fund-cf-data-store', data=None),
        dcc.Store(id='cf-edited-data-store', data={})
    ])
...
def pipeline_page():
    return html.Div([
        ...
        dbc.Modal([
            ...
            dbc.ModalBody([
                dcc.Store(id='edit-pipe-index-store', data=None),
                dbc.Row([
                    dbc.Col([dbc.Label("Deal Name"), dbc.Input(id="edit-pipe-name")], width=6),
                    dbc.Col([dbc.Label("Deal Type"), dbc.Select(id="edit-pipe-type", options=[
                        {"label": "GP-Led (Multi-Asset)", "value": "GP-Led (Multi-Asset)"},
                        {"label": "GP-Led (Single-Asset)", "value": "GP-Led (Single-Asset)"},
                        {"label": "Diversified LP-Led", "value": "Diversified LP-Led"},
                        {"label": "Co-Investments", "value": "Co-Investments"},
                ...
                dbc.Row([
                    dbc.Col([dbc.Label("Target Size ($mm)"), dbc.Input(id="edit-pipe-size", type="number", step=0.1)],
                            width=4),
                    dbc.Col([dbc.Label("Target IRR"), dbc.Input(id="edit-pipe-irr", type="number", step=0.01)],
                            width=4),
                    dbc.Col([dbc.Label("Stage"), dbc.Select(id="edit-pipe-stage", options=[
                        {"label": "Screening", "value": "Screening"},
                        {"label": "Due Diligence", "value": "Due Diligence"},
                        {"label": "Term Sheet", "value": "Term Sheet"},
                        {"label": "Final Docs", "value": "Final Docs"},
                ...
                dbc.Row([
                    dbc.Col([dbc.Label("Probability (%)"),
                             dbc.Input(id="edit-pipe-prob", type="number", step=1, placeholder="75")], width=6),
                    dbc.Col([dbc.Label("Expected Close Month"),
                             dbc.Select(id="edit-pipe-month", options=generate_month_options(), value=0)], width=6),
                ], className="mb-3"),
            ]),
            dbc.ModalFooter([
                dbc.Button("Cancel", id="btn-cancel-edit-pipe", color="secondary", className="me-2"),
                dbc.Button("Save Changes", id="btn-save-edit-pipe", color="info")
            ])
...
def populate_edit_modal(n_clicks, deals, fund_cf_data):
    ...
    if fund_cf_data:
        ...
        # Find this deal in cashflows and get latest NAV
        for cf_deal in fund_cf_data:
            if cf_deal['name'] == deal['name']:
                # Prefer nav_series (latest month-end) over static nav field
                nav_series = cf_deal.get('nav_series', [])
                latest_nav = 0
                for v in reversed(nav_series):
                    if v and v != 0:
                        latest_nav = float(v)
                        break
                if latest_nav > 0:
                    nav = latest_nav
                    nav_source = "✓ Auto-pulled from latest month-end (Deal Cashflows)"
                elif cf_deal.get('nav', 0) > 0:
                    nav = cf_deal['nav']
                    nav_source = "✓ Auto-pulled from latest cashflows"
            ...
            dbc.Row([
                ...
                dbc.Col([
                    html.Div([
                        ...
                        html.Br(),
                        html.Strong("NAV: ", style={'fontFamily': C['sans'], 'color': C['muted'], 'fontSize': '11px'}),
...
def update_dashboard_kpis(deals, placeholders, config, liquidity_data, fund_cf_data):
    ...
    # Dry powder calculation - prefer Dec 2026 liquidity surplus from uploaded file
    dry_powder = config['fund_parameters']['dry_powder']
    if liquidity_data and liquidity_data.get('dec_2026_dry_powder'):
        dry_powder = liquidity_data['dec_2026_dry_powder']
...
@app.callback(
    [Output('port-nav', 'children'), Output('port-total-commitment', 'children'),
     Output('port-dry-powder', 'children'),
     Output('port-total-fund', 'children'), Output('port-target-return', 'children')],
    [Input('deals-store', 'data'), Input('config-store', 'data'),
     Input('fund-cf-data-store', 'data'), Input('liquidity-data-store', 'data')]
)
def update_portfolio_metrics(deals, config, fund_cf_data, liquidity_data):
    m = calculate_portfolio_metrics(deals)

    # Pull Current NAV from Deal Cashflows tab (latest month-end NAV)
    current_nav = m['total_nav']
    if fund_cf_data:
        # Sum the most recent non-zero NAV value across all deals
        nav_sum = 0
        for deal in fund_cf_data:
            nav_series = deal.get('nav_series', [])
            # Find last non-zero value
            for val in reversed(nav_series):
                if val and val != 0:
                    nav_sum += float(val)
                    break
        if nav_sum > 0:
    ...
    # Total Commitment from all deals
    total_commitment = sum(d.get('total_commitment', d.get('commitment', d.get('size', 0))) for d in (deals or []))

    # Dry Powder: from liquidity file Dec 2026 row 53 col 13, else config
    dry_powder = config['fund_parameters']['dry_powder']
    if liquidity_data and liquidity_data.get('dec_2026_dry_powder'):
        dry_powder = liquidity_data['dec_2026_dry_powder']

    req_irr = calculate_required_future_irr(m['weighted_irr'], current_nav, dry_powder, config)
    return (
        f"${current_nav:.1f}M",
        f"${total_commitment:.1f}M",
        f"${dry_powder:.1f}M",
        f"${current_nav + dry_powder:.0f}M",
        f"{req_irr:.1%}"
    )
...
def upload_fund_cf_file(contents, filename):
    ...
    try:
        ...
        if is_csv:
            ...
            # Find section column starts from row 1 (template has section labels in row 1)
            section_headers = {}
            for col in range(1, ws.max_column + 1):
                val = ws.cell(1, col).value
                if val:
                    key = str(val).strip()
                    if key in ('Net CF', 'Called Capital', 'Capital Calls', 'Distributions', 'NAV'):
                        section_headers[key] = col

            # Find header row (column-name row)
            header_row = None
            ...
            if header_row:
                ...
                # Merge section header columns into headers dict
                headers.update(section_headers)

                print(f"Found headers: {list(headers.keys())[:15]}")
                print(f"Section columns: {section_headers}")

                # Parse each deal row
                for row in range(header_row + 1, min(header_row + 100, ws.max_row + 1)):
                    # Get deal name from AIC Name or Investments column
                    deal_name = None
                    ...
                    # Investment Type
                    if 'Investment Type' in headers:
                        val = ws.cell(row, headers['Investment Type']).value
                        if val:
                            deal_type = str(val)

                    # AIC / Manager Name
                    aic_name = ''
                    if 'AIC Name' in headers:
                        val = ws.cell(row, headers['AIC Name']).value
                        if val:
                            aic_name = str(val)

                    # Commitment ($m) - column 11
                    if 'Commitment ($m)' in headers:
                        try:
                            val = ws.cell(row, headers['Commitment ($m)']).value
                            if val and isinstance(val, (int, float)):
                                commitment = float(val)
                        except:
                            pass
                    ...
                    if 'Paid In' in headers:
                        try:
                            val = ws.cell(row, headers['Paid In']).value
                            if val and isinstance(val, (int, float)):
                                paid_in = float(val)
                        except:
                            pass
                    ...
                    if 'Unfunded' in headers:
                        try:
                            val = ws.cell(row, headers['Unfunded']).value
                            if val and isinstance(val, (int, float)):
                                unfunded = float(val)
                        except:
                            pass

                    # Calculate current commitment from paid in or unfunded
                    current_commitment = paid_in if paid_in > 0 else (
                                commitment - unfunded / 1000000) if unfunded > 0 else commitment

                    # --- Extract time-series for Net CF, Calls, Dists, NAV ---
                    # Template structure: 4 sections side-by-side, each ~175 columns wide.
                    # Row 1 col 14 = "Net CF" header; Calls ~col 190; Dists ~col 365; NAV ~col 537
                    # We detect by scanning the header row for section labels.
                    net_cf_col_start = headers.get('Net CF', 14)
                    calls_col_start = headers.get('Called Capital',
                                                  headers.get('Capital Calls', net_cf_col_start + 175))
                    dists_col_start = headers.get('Distributions', calls_col_start + 175)
                    nav_col_start = headers.get('NAV', dists_col_start + 175)

                    NUM_MONTHS = 170  # up to ~14 years of monthly data

                    def _extract_series(col_start, length):
                        series = []
                        for c in range(col_start, col_start + length):
                            try:
                                v = ws.cell(row, c).value
                                series.append(float(v) if v and isinstance(v, (int, float)) else 0.0)
                            except:
                                series.append(0.0)
                        return series

                    net_cf_series = _extract_series(net_cf_col_start, NUM_MONTHS)
                    calls_series = _extract_series(calls_col_start, NUM_MONTHS)
                    dists_series = _extract_series(dists_col_start, NUM_MONTHS)
                    nav_series = _extract_series(nav_col_start, NUM_MONTHS)

                    # Latest NAV = last non-zero in nav_series
                    for v in reversed(nav_series):
                        if v and v != 0:
                            nav = v / 1000000 if abs(v) > 1000 else v
                            break
                    if nav == 0:
                        # Try scanning backwards from last column
                        for col in range(min(ws.max_column, nav_col_start + NUM_MONTHS), nav_col_start - 1, -1):
                            try:
                                val = ws.cell(row, col).value
                                if val and isinstance(val, (int, float)) and val != 0:
                                    nav = float(val) / 1000000
                                    break
                            except:
                                continue
                    if nav == 0:
                    ...
                    # Convert large absolute values (stored in units) to $mm
                    def _scale_series(s):
                        # If values look like raw $, scale to $mm
                        maxv = max((abs(v) for v in s if v), default=0)
                        if maxv > 10000:
                            return [v / 1_000_000 for v in s]
                        return s

                    net_cf_series = _scale_series(net_cf_series)
                    calls_series = _scale_series(calls_series)
                    dists_series = _scale_series(dists_series)
                    nav_series = _scale_series(nav_series)
                    ...
                    deal_data = {
                        ...
                        'paid_in': paid_in,
                        'nav': nav,
                        'unfunded': unfunded / 1000000 if unfunded > 1000 else unfunded,
                        'net_cf_series': net_cf_series,
                        'calls_series': calls_series,
                        'dists_series': dists_series,
                        'nav_series': nav_series,
                        'monthly_cfs': {}
                    }

                    print(
                        f"Parsed: {deal_name} | Commitment: {commitment} | NAV: {nav} | Unfunded: {deal_data['unfunded']}")
...
# Helper: build a styled DataTable for a CF section
def _build_cf_datatable(table_id, table_rows, months, info_cols, editable_cols=None):
    """Build a DataTable for one cashflow section (Net CF, Calls, Dists, or NAV)"""
    if not table_rows:
        return html.P("No data available", className="text-muted")

    all_columns = info_cols + months
    col_defs = []
    ...
    # Style: highlight totals row differently
    cond = [
        {'if': {'row_index': 'odd'}, 'backgroundColor': C['surface']},
        {'if': {'filter_query': '{Deal} = "TOTAL"'}, 'backgroundColor': C['border2'], 'fontWeight': 'bold',
         'color': C['sky']},
    ]
    ...
    return dash_table.DataTable(
        ...
        columns=col_defs,
        editable=True,
        style_cell={'textAlign': 'right', 'padding': '8px 12px', 'fontFamily': C['mono'],
                    'fontSize': '11px', 'minWidth': '80px', 'maxWidth': '130px'},
        style_cell_conditional=[
            {'if': {'column_id': 'Deal'}, 'textAlign': 'left', 'minWidth': '160px', 'maxWidth': '200px',
             'fontWeight': 'bold'},
            {'if': {'column_id': 'AIC'}, 'textAlign': 'left', 'minWidth': '120px'},
            {'if': {'column_id': 'Type'}, 'textAlign': 'left', 'minWidth': '110px'},
        ],
        style_header={'backgroundColor': C['surface'], 'color': C['text'], 'fontWeight': 'bold',
                      'border': f'1px solid {C["border"]}', 'position': 'sticky', 'top': 0},
        style_data={'backgroundColor': C['panel'], 'color': C['text'], 'border': f'1px solid {C["border"]}'},
        style_data_conditional=cond,
        style_table={'overflowX': 'auto', 'maxHeight': '400px', 'overflowY': 'auto'},
        fixed_columns={'headers': True, 'data': len(info_cols)},
...
def _fmt(val):
    """Format a number for cashflow display"""
    if val is None or val == 0:
        return "-"
    return f"{val:.2f}"


def _get_cf_tables_data(deals, uploaded_data, num_months):
    """Build row data for all 4 CF tables"""
    base_date = datetime(2026, 1, 1)
    months = [(base_date + relativedelta(months=i)).strftime('%b-%y') for i in range(int(num_months))]

    info_cols = ['Deal', 'AIC', 'Type', 'Commitment ($m)', 'Paid In', 'Unfunded']
    data_source = uploaded_data if uploaded_data else deals

    net_rows, calls_rows, dists_rows, nav_rows = [], [], [], []
    net_totals = [0] * int(num_months)
    calls_totals = [0] * int(num_months)
    dists_totals = [0] * int(num_months)
    nav_totals = [0] * int(num_months)

    for deal in (data_source or []):
        if uploaded_data:
            name = deal.get('name', 'N/A')
            aic = deal.get('aic', deal.get('manager', 'N/A'))
            dtype = deal.get('type', deal.get('strategy', 'N/A'))
            commitment = deal.get('commitment', 0)
            paid_in = deal.get('paid_in', deal.get('size', 0))
            unfunded = deal.get('unfunded', commitment - paid_in)
            net_cf_series = deal.get('net_cf_series', [0] * int(num_months))
            calls_series = deal.get('calls_series', [0] * int(num_months))
            dists_series = deal.get('dists_series', [0] * int(num_months))
            nav_series = deal.get('nav_series', [0] * int(num_months))
        else:
            name = deal.get('name', 'N/A')
            aic = deal.get('manager', 'N/A')
            dtype = deal.get('strategy', 'N/A')
            commitment = deal.get('total_commitment', deal.get('commitment', deal.get('size', 0)))
            paid_in = deal.get('current_commitment', deal.get('size', 0))
            unfunded = deal.get('unfunded', commitment - paid_in)
            net_cf_series = [0] * int(num_months)
            calls_series = [0] * int(num_months)
            dists_series = [0] * int(num_months)
            nav_series = [deal.get('nav', deal.get('size', 0))] + [0] * (int(num_months) - 1)

        base_info = {
            'Deal': name, 'AIC': aic, 'Type': dtype,
            'Commitment ($m)': _fmt(commitment), 'Paid In': _fmt(paid_in), 'Unfunded': _fmt(unfunded)
        }

        def _pad(series, length):
            lst = list(series) if series else []
            return lst + [0] * (length - len(lst))

        nc = _pad(net_cf_series, int(num_months))
        cc = _pad(calls_series, int(num_months))
        dd = _pad(dists_series, int(num_months))
        nv = _pad(nav_series, int(num_months))

        nr, cr, dr, vr = {**base_info}, {**base_info}, {**base_info}, {**base_info}
        for i, m in enumerate(months):
            nr[m] = _fmt(nc[i]);
            calls_totals[i] += (cc[i] or 0);
            net_totals[i] += (nc[i] or 0)
            cr[m] = _fmt(cc[i]);
            dists_totals[i] += (dd[i] or 0)
            dr[m] = _fmt(dd[i]);
            nav_totals[i] += (nv[i] or 0)
            vr[m] = _fmt(nv[i])
        net_rows.append(nr);
        calls_rows.append(cr);
        dists_rows.append(dr);
        nav_rows.append(vr)

    # Totals row
    def _total_row(totals):
        r = {'Deal': 'TOTAL', 'AIC': '', 'Type': '', 'Commitment ($m)': '', 'Paid In': '', 'Unfunded': ''}
        for i, m in enumerate(months):
            r[m] = _fmt(totals[i])
        return r

    net_rows.append(_total_row(net_totals))
    calls_rows.append(_total_row(calls_totals))
    dists_rows.append(_total_row(dists_totals))
    nav_rows.append(_total_row(nav_totals))

    return net_rows, calls_rows, dists_rows, nav_rows, months, info_cols


# Four separate cashflow table callbacks
@app.callback(
    [Output('cf-netcf-table', 'children'), Output('cf-calls-table', 'children'),
     Output('cf-dists-table', 'children'), Output('cf-nav-table', 'children')],
    [Input('deals-store', 'data'), Input('fund-cf-data-store', 'data'),
     Input('cf-view-period', 'value'), Input('cf-type-view', 'value')]
)
def generate_cf_tables(deals, uploaded_data, num_months, cf_type):
    """Generate the 4 separate Fund Level CF tables: Net CF, Calls, Dists, NAV"""
    data_source = uploaded_data if uploaded_data else deals
    if not data_source:
        empty = html.P("Upload Fund Level CF Excel file or add deals to view cashflows", className="text-muted")
        return empty, empty, empty, empty

    net_rows, calls_rows, dists_rows, nav_rows, months, info_cols = _get_cf_tables_data(deals, uploaded_data,
                                                                                        num_months)

    months_editable = months  # all month columns are editable

    net_tbl = _build_cf_datatable('cf-netcf-datatable', net_rows, months, info_cols, months_editable)
    calls_tbl = _build_cf_datatable('cf-calls-datatable', calls_rows, months, info_cols, months_editable)
    dists_tbl = _build_cf_datatable('cf-dists-datatable', dists_rows, months, info_cols, months_editable)
    nav_tbl = _build_cf_datatable('cf-nav-datatable', nav_rows, months, info_cols, months_editable)

    return net_tbl, calls_tbl, dists_tbl, nav_tbl
...
def upload_liquidity_file(contents, filename):
    ...
    try:
        ...
        # DEC 2026 DRY POWDER - Row 53, Column 13 (Min Deployable Capital - Liquidity Surplus - post Pipeline)
        # Date headers start at col 4 (Mar 2026), so Dec 2026 = col 4 + 9 = col 13
        dec_2026_val = ws.cell(53, 13).value
        if dec_2026_val and isinstance(dec_2026_val, (int, float)):
            liquidity_data['dec_2026_dry_powder'] = float(dec_2026_val) / 1_000_000
        else:
            # Fallback: scan row 28 for Dec 2026 date and use that column
            import datetime as dt
            for col in range(4, 30):
                header = ws.cell(28, col).value
                if isinstance(header, dt.datetime) and header.year == 2026 and header.month == 12:
                    dec_val = ws.cell(53, col).value
                    if dec_val and isinstance(dec_val, (int, float)):
                        liquidity_data['dec_2026_dry_powder'] = float(dec_val) / 1_000_000
                    break
...
# ==================== EDIT PLACEHOLDER CALLBACKS ====================

@app.callback(
    Output('modal-edit-placeholder', 'is_open'),
    [Input({'type': 'edit-placeholder', 'index': ALL}, 'n_clicks'),
     Input('btn-cancel-edit-ph', 'n_clicks'), Input('btn-save-edit-ph', 'n_clicks')],
    State('modal-edit-placeholder', 'is_open'), prevent_initial_call=True
)
def toggle_edit_ph_modal(edit_clicks, cancel, save, is_open):
    ctx = callback_context
    if not ctx.triggered: return is_open
    triggered = ctx.triggered[0]['prop_id']
    if 'edit-placeholder' in triggered and any(edit_clicks): return True
    return False


@app.callback(
    [Output('edit-ph-name', 'value'), Output('edit-ph-strategy', 'value'),
     Output('edit-ph-size', 'value'), Output('edit-ph-irr', 'value'),
     Output('edit-ph-month', 'value'), Output('edit-ph-segment', 'value'),
     Output('edit-ph-index-store', 'data')],
    Input({'type': 'edit-placeholder', 'index': ALL}, 'n_clicks'),
    State('placeholder-deals-store', 'data'), prevent_initial_call=True
)
def populate_edit_ph_modal(n_clicks, placeholders):
    if not any(n_clicks) or not placeholders: return "", "", 0, 0.20, 0, "Seed", None
    ctx = callback_context
    if not ctx.triggered: return "", "", 0, 0.20, 0, "Seed", None
    button_info = json.loads(ctx.triggered[0]['prop_id'].split('.')[0])
    idx = button_info['index']
    if idx < 0 or idx >= len(placeholders): return "", "", 0, 0.20, 0, "Seed", None
    ph = placeholders[idx]
    return (ph.get('name', ''), ph.get('strategy', ''), ph.get('size', 0),
            ph.get('target_irr', ph.get('irr', 0.20)), ph.get('expected_month', 0),
            ph.get('segment', 'Seed'), idx)


@app.callback(
    Output('placeholder-deals-store', 'data', allow_duplicate=True),
    Input('btn-save-edit-ph', 'n_clicks'),
    [State('placeholder-deals-store', 'data'), State('edit-ph-index-store', 'data'),
     State('edit-ph-name', 'value'), State('edit-ph-strategy', 'value'),
     State('edit-ph-size', 'value'), State('edit-ph-irr', 'value'),
     State('edit-ph-month', 'value'), State('edit-ph-segment', 'value')],
    prevent_initial_call=True
)
def save_edit_ph(n, placeholders, idx, name, strategy, size, irr, month, segment):
    if not n or idx is None or not placeholders: return placeholders or []
    ph = placeholders[idx]
    ph.update({'name': name or ph['name'], 'strategy': strategy or ph['strategy'],
               'size': float(size) if size else ph['size'],
               'target_irr': float(irr) if irr else ph.get('target_irr', 0.20),
               'expected_month': int(month) if month is not None else ph.get('expected_month', 0),
               'segment': segment or ph.get('segment', 'Seed')})
    placeholders[idx] = ph
    return placeholders


# ==================== EDIT PIPELINE CALLBACKS ====================

@app.callback(
    Output('modal-edit-pipeline', 'is_open'),
    [Input({'type': 'edit-pipeline', 'index': ALL}, 'n_clicks'),
     Input('btn-cancel-edit-pipe', 'n_clicks'), Input('btn-save-edit-pipe', 'n_clicks')],
    State('modal-edit-pipeline', 'is_open'), prevent_initial_call=True
)
def toggle_edit_pipe_modal(edit_clicks, cancel, save, is_open):
    ctx = callback_context
    if not ctx.triggered: return is_open
    triggered = ctx.triggered[0]['prop_id']
    if 'edit-pipeline' in triggered and any(edit_clicks): return True
    return False


@app.callback(
    [Output('edit-pipe-name', 'value'), Output('edit-pipe-type', 'value'),
     Output('edit-pipe-size', 'value'), Output('edit-pipe-irr', 'value'),
     Output('edit-pipe-stage', 'value'), Output('edit-pipe-prob', 'value'),
     Output('edit-pipe-month', 'value'), Output('edit-pipe-index-store', 'data')],
    Input({'type': 'edit-pipeline', 'index': ALL}, 'n_clicks'),
    State('pipeline-store', 'data'), prevent_initial_call=True
)
def populate_edit_pipe_modal(n_clicks, pipeline):
    if not any(n_clicks) or not pipeline: return "", "", 0, 0.20, "Screening", 75, 0, None
    ctx = callback_context
    if not ctx.triggered: return "", "", 0, 0.20, "Screening", 75, 0, None
    button_info = json.loads(ctx.triggered[0]['prop_id'].split('.')[0])
    idx = button_info['index']
    if idx < 0 or idx >= len(pipeline): return "", "", 0, 0.20, "Screening", 75, 0, None
    pipe = pipeline[idx]
    prob_raw = pipe.get('probability', 0.75)
    prob_pct = int(prob_raw * 100) if prob_raw <= 1 else int(prob_raw)
    return (pipe.get('name', ''), pipe.get('type', pipe.get('strategy', '')),
            pipe.get('size', 0), pipe.get('target_irr', pipe.get('irr', 0.20)),
            pipe.get('stage', 'Screening'), prob_pct,
            pipe.get('expected_month', pipe.get('month', 0)), idx)


@app.callback(
    Output('pipeline-store', 'data', allow_duplicate=True),
    Input('btn-save-edit-pipe', 'n_clicks'),
    [State('pipeline-store', 'data'), State('edit-pipe-index-store', 'data'),
     State('edit-pipe-name', 'value'), State('edit-pipe-type', 'value'),
     State('edit-pipe-size', 'value'), State('edit-pipe-irr', 'value'),
     State('edit-pipe-stage', 'value'), State('edit-pipe-prob', 'value'),
     State('edit-pipe-month', 'value')],
    prevent_initial_call=True
)
def save_edit_pipe(n, pipeline, idx, name, ptype, size, irr, stage, prob, month):
    if not n or idx is None or not pipeline: return pipeline or []
    pipe = pipeline[idx]
    pipe.update({'name': name or pipe['name'],
                 'type': ptype or pipe.get('type', pipe.get('strategy', '')),
                 'strategy': ptype or pipe.get('strategy', ''),
                 'size': float(size) if size else pipe['size'],
                 'target_irr': float(irr) if irr else pipe.get('target_irr', 0.20),
                 'stage': stage or pipe.get('stage', 'Screening'),
                 'probability': float(prob) / 100 if prob and float(prob) > 1 else float(prob) if prob else pipe.get(
                     'probability', 0.75),
                 'expected_month': int(month) if month is not None else pipe.get('expected_month', 0)})
    pipeline[idx] = pipe
    return pipeline
