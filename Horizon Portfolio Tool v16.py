...
def cashflows_page():
    return html.Div([
        ...
        dbc.Alert([
            html.I(className="fas fa-info-circle me-2"),
            "Monthly cashflows by deal. Each row = one deal. Columns = months from Jan 2026 onwards. Shows Calls, Distributions, Net CF, and ending NAV."
        ], color="info", className="mb-4"),
...
def liquidity_assumptions_page():
    return html.Div([
        ...
        dbc.Alert([
            ...
            "Current liquidity position, near-term flows, and NAV projections"
        ], color="info", className="mb-4"),
...
@app.callback(
    ...
    [Input('deals-store', 'data'), Input('cf-view-period', 'value'),
     Input('cf-type-view', 'value'), Input('cashflows-store', 'data')]
)
def generate_monthly_cf_table(deals, num_months, cf_type, cashflows_data):
    """Generate Fund Level CF table like Excel - each row is a deal, columns are months"""

    if not deals:
        return html.P("Add deals to see monthly cashflows", className="text-muted")
    ...
    for deal in deals:
        ...
        row_data = {
            ...
            'Paid In': f"${deal['size']:.1f}M",
            'Unfunded': f"${max(0, deal.get('commitment', deal['size']) - deal['size']):.1f}M",
        }

        # Add monthly columns
        # For now showing placeholder - you'd calculate actual from cashflows
        for month in months:
            # Calculate net CF for this month
            # In full version, would pull from cashflows_data
            row_data[month] = "$0.0M"  # Placeholder
    ...
    all_columns = ['Deal', 'Type', 'Status', 'Commitment', 'Paid In', 'Unfunded'] + months
...
def generate_liquidity_waterfall(deals):
    """Generate liquidity waterfall like Excel"""

    # Calculate key metrics
    total_nav = sum(d['size'] for d in deals) if deals else 0
    current_cash = 0.88  # Placeholder
    glf_balance = 19.95  # Placeholder
    cqs_balance = 17.27  # Placeholder
...
def generate_near_term_flows(deals):
    """Generate near term flows table (next 12 months)"""

    base_date = datetime(2026, 3, 31)  # Current quarter end
    months = []
    subscriptions = []
    redemptions = []
    portfolio_flows = []
    ...
    for i in range(12):
        month_date = base_date + relativedelta(months=i)
        months.append(month_date.strftime('%b-%y'))
        # Placeholders - would calculate from actual data
    ...
    # Create table
    table_data = [
        {'Flow Type': 'Subscriptions', **{months[i]: f'${subscriptions[i]:.1f}M' for i in range(12)}},
        {'Flow Type': 'Redemptions', **{months[i]: f'${redemptions[i]:.1f}M' for i in range(12)}},
        {'Flow Type': 'Portfolio Net Flows', **{months[i]: f'${portfolio_flows[i]:.1f}M' for i in range(12)}},
    ]
...
def generate_nav_projection_chart(deals):
    """Generate NAV end projections chart"""

    base_nav = sum(d['size'] for d in deals) if deals else 0
