...
def add_or_update_portfolio(_, portfolio, nid, edit_idx,
    ...
    if edit_idx is not None and 0 <= edit_idx < len(p):
        ...
        p[edit_idx] = deal
        # Clear edit-idx and selection so next click is a fresh add
...
def populate_edit(btn_click, trigger_idx, selected_idx, portfolio):
    ...
    # If triggered from Pro Forma route, use trigger_idx directly
    if "populate-port-trigger" in triggered and trigger_idx is not None:
        idx = trigger_idx
    else:
        idx = selected_idx
        if idx is None:
            return [no_update]*19 + ["⚠ Select a deal row first."]

    if idx is None or idx >= len(portfolio):
        return [no_update]*19 + ["⚠ No row selected — click a row in the table first."]
    d = portfolio[idx]
    return (
        idx,
...
def populate_pipe_edit(btn_click, trigger_idx, selected_idx, pipeline):
    ...
    if "populate-pipe-trigger" in triggered and trigger_idx is not None:
        ...
        idx = selected_idx
        if idx is None:
            return [no_update]*10 + ["⚠ Select a deal row first."]
    if idx >= len(pipeline):
        return [no_update]*11
    d = pipeline[idx]
    return (
...
def populate_ph_edit(_, selected_idx, placeholders):
    if not placeholders:
        return [no_update]*9
    if selected_idx is None:
        return [no_update]*8 + ["⚠ Select a row first."]
    idx = selected_idx
