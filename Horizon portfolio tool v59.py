...
app.layout = dbc.Container([
    ...
    _hidden_callback_placeholders
], fluid=True, style={'backgroundColor': C['bg'], 'minHeight': '100vh'})



# Hidden placeholders for dynamic tab/page components referenced by callbacks
_hidden_callback_placeholders = html.Div([
    html.Div(id='liq-strategy-assumptions', style={'display': 'none'}),
    html.Div(id='liq-summary', style={'display': 'none'}),
    html.Div(id='pf-comparison-table', style={'display': 'none'}),
    html.Div(id='pf-scenario-deals', style={'display': 'none'}),
    dcc.Graph(id='pf-concentration-chart', style={'display': 'none'}),
    dcc.Graph(id='pf-strategy-chart', style={'display': 'none'}),
    dcc.Dropdown(id='pf-deal-select', options=[], style={'display': 'none'}),
    dbc.Button("hidden", id='btn-add-to-pf', style={'display': 'none'}),
    dbc.Button("hidden", id='btn-reset-pf', style={'display': 'none'}),
    dbc.Input(id='pf-custom-name', style={'display': 'none'}),
    dbc.Input(id='pf-custom-size', type='number', style={'display': 'none'}),
    dbc.Input(id='pf-custom-irr', type='number', style={'display': 'none'}),
], style={'display': 'none'})
...
app.validation_layout = dbc.Container([
    ...
    dcc.Store(id='deals-store'),
    dcc.Store(id='placeholder-deals-store'),
    dcc.Store(id='pipeline-store'),
    dcc.Store(id='cashflows-store'),
    dcc.Store(id='fund-cf-data-store'),
    dcc.Store(id='liquidity-data-store'),
    dcc.Store(id='liquidity-dry-powder-store'),
    dcc.Store(id='proforma-scenario-store'),
    dcc.Store(id='liquidity-assumptions-store'),
...
@app.callback(
    [Output('config-store', 'data', allow_duplicate=True), Output('settings-save-status', 'children'), Output('save-status', 'children', allow_duplicate=True)],
...
def save_or_reset_settings(save_clicks, reset_clicks, dry_powder, twr, hold, fee, carry, hurdle, liq, loss,
    ...
    if trigger == 'btn-reset-settings':
        return json.loads(json.dumps(DEFAULT_CONFIG)), dbc.Alert("Settings reset to defaults.", color="secondary", className="mb-0"), 'settings-reset'
...
@app.callback(
    [Output('config-store', 'data', allow_duplicate=True), Output('overview-save-status', 'children'), Output('save-status', 'children', allow_duplicate=True)],
...
def upload_fund_cf_file(contents, filename):
    """Parse uploaded Fund Level CF file and store the workbook-derived cashflow data."""
    if not contents:
        raise dash.exceptions.PreventUpdate
    ...
    try:
        ...
        _, content_string = parts
        decoded = base64.b64decode(content_string)

        fname = (filename or '').lower()
        if fname.endswith('.csv'):
            raise ValueError("CSV upload is not supported for the Fund Level CF template. Please upload the Excel template (.xlsx or .xlsm).")

        deals_data = parse_fund_cf_workbook(BytesIO(decoded))

        if not deals_data:
            raise ValueError("The workbook was read, but no Fund Level CF sections were found. Check that row 1 contains Net CF, Called Capital, Distributions and NAV blocks.")

        success_msg = dbc.Alert([
            html.I(className="fas fa-check-circle me-2"),
            html.Span(f"Uploaded: {filename}"),
            html.Br(),
            html.Small(f"{len(deals_data)} rows parsed from the Fund Level CF workbook.", className="text-muted")
        ...
        error_msg = dbc.Alert([
            ...
            html.Div([
                f"Error uploading {filename or 'file'}",
                html.Br(),
                html.Small(str(e), className="text-muted"),
                html.Br(),
                html.Small("Please upload the Excel Fund Level CF template (.xlsx or .xlsm).", className="text-muted")
            ])
