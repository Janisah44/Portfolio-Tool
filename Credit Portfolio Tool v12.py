...
# ── Persistence ──────────────────────────────────────────────────────────────
DATA_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "credit_portfolio_tool_v12_data.pkl")
...
SEED_PORTFOLIO = [
    dict(id=1, name="Aquila Credit Opportunities", manager="Aquila Capital", deal_type="LP-Led Secondary", region="Europe", sector="Diversified", commitment=65,
    ...
    dict(id=2, name="Brookfield Credit Continuation", manager="Brookfield", deal_type="GP-Led Multi Asset CV", region="North America", sector="Specialty Finance", commitment=90,
    ...
    dict(id=3, name="Mercury Senior Credit", manager="Mercury Partners", deal_type="Co-Investment", region="United Kingdom", sector="Industrial", commitment=80,
    ...
    dict(id=4, name="Northlight Capital CV", manager="Northlight", deal_type="GP-Led Single Asset CV", region="Global", sector="Financials", commitment=55,
...
SEED_PIPELINE = [
    dict(id=10, name="Project Cedar", manager="Cedar Ridge", deal_type="GP-Led Multi Asset CV", region="Europe", sector="Financials", commitment=45,
    ...
    dict(id=11, name="Project Juniper", manager="Juniper Credit", deal_type="LP-Led Secondary", region="North America", sector="Healthcare", commitment=70,
    ...
    dict(id=12, name="Project Quartz", manager="Quartz Partners", deal_type="Co-Investment", region="United Kingdom", sector="Real Estate", commitment=50,
...
def aggregate_metrics(portfolio, pipeline, config):
    ...
    return {
        ...
        "pipeline_pro_forma_nav": sum(float(d.get("pro_forma_nav", d.get("current_nav", d.get("commitment", 0))) or 0) for d in pipeline),
...
# ── App ──────────────────────────────────────────────────────────────────────
app = dash.Dash(__name__, title="Credit Portfolio Tool v12", suppress_callback_exceptions=True)
    ...
    html.Div([
        html.Div([
            html.Div([
                ...
                html.Span("Institutional Dashboard v12", style=dict(fontSize=18, fontWeight=300, color=C["muted"], fontFamily=C["sans"])),
            ]),
...
def portfolio_table_rows(portfolio, config):
    ...
    for d in portfolio:
        ...
        rows.append({
            "ID": d["id"], "Deal": d["name"], "Manager": d.get("manager", ""), "Type": d["deal_type"], "Region": d.get("region", ""), "Sector": d["sector"],
            "Current Commit": fmt_money(d.get("current_commitment", d["commitment"])), "Current NAV": fmt_money(d.get("current_nav", d["commitment"])),
            "Orig Legal Commit": fmt_money(d.get("original_legal_commitment", d["commitment"])), "Price/NAV": f"{float(d.get('price_to_nav', 100) or 0):.1f}%",
            "Weight": f"{d['commitment']/total*100:.1f}%", "Yield": f"{d['all_in_yield']:.1f}%", "Spread": f"{d['spread_bps']:.0f}",
...
def tab_portfolio(portfolio, config):
    ...
    form = html.Div([
        ...
        ], style=dict(display="grid", gridTemplateColumns="1.6fr 1.3fr 1.2fr 1.2fr .9fr .9fr .7fr .7fr .8fr .8fr .8fr .9fr 1fr .8fr auto", gap=8, alignItems="end")),
...
def tab_pipeline(pipeline, portfolio, config):
    ...
    for d in pipeline:
        ann = ann_return_from_moic(d["moic"], d["hold_years"])
        rows.append({
            "ID": d["id"], "Deal": d["name"], "Manager": d.get("manager", ""), "Type": d["deal_type"], "Region": d.get("region", ""),
            ...
            "Current Commit": fmt_money(d.get("current_commitment", d["commitment"])), "Current NAV": fmt_money(d.get("current_nav", d["commitment"])),
            "Price/NAV": f"{float(d.get('price_to_nav', 100) or 0):.1f}%", "Pro Forma NAV": fmt_money(d.get("pro_forma_nav", d.get("current_nav", d["commitment"]))),
            "Yield": f"{d['all_in_yield']:.1f}%", "Spread": f"{d['spread_bps']:.0f}", "Duration": f"{modified_duration(d, config.get('target_net_return', 11)):.2f}y",
    ...
    form = html.Div([
        ...
        ], style=dict(display="grid", gridTemplateColumns="1.5fr 1.2fr 1.1fr 1.1fr .9fr .8fr .9fr .9fr .7fr .7fr .7fr .7fr .7fr .8fr .8fr .8fr auto", gap=8, alignItems="end")),
...
def tab_analytics(portfolio, pipeline, config):
    ...
    manager_exp = df.groupby("manager", as_index=False)["commitment"].sum().sort_values("commitment", ascending=False).head(10)
    fig_manager = go.Figure(go.Bar(x=manager_exp["manager"], y=manager_exp["commitment"], marker_color=C["blue"], text=[fmt_money(v) for v in manager_exp["commitment"]], textposition="outside", textfont_color=C["text"]))
    ...
    fig_region = go.Figure(go.Pie(labels=region_exp["region"], values=region_exp["commitment"], hole=0.55, marker_colors=[C["purple"], C["teal"], C["sky"], C["amber"], C["pink"]], textfont_color=C["text"]))
    fig_region.update_layout(**CHART, height=320, title="Exposure by Region")

    discount_by_type = df.groupby("deal_type").apply(lambda x: (x["price_to_nav"] * x["current_nav"]).sum() / max(x["current_nav"].sum(), 1)).reset_index(name="blended_price_to_nav")
    fig_discount = go.Figure(go.Bar(x=discount_by_type["deal_type"], y=discount_by_type["blended_price_to_nav"], marker_color=C["amber"], text=[f"{v:.1f}%" for v in discount_by_type["blended_price_to_nav"]], textposition="outside", textfont_color=C["text"]))
    ...
    if not pipe.empty:
        ...
        fig_bridge.add_trace(go.Bar(x=bridge["Deal"], y=bridge["Current NAV"], name="Current NAV", marker_color=C["sky"]))
        fig_bridge.add_trace(go.Bar(x=bridge["Deal"], y=bridge["Uplift"], name="PF NAV uplift", marker_color=C["green"]))
        fig_bridge.update_layout(**CHART, barmode="relative", height=340, title="Pipeline Current NAV to Pro Forma NAV Bridge", yaxis_title="$M")
    else:
        fig_bridge = go.Figure()
...
def tab_returns(portfolio, pipeline, config):
    ...
    scatter.add_trace(go.Scatter(
        ...
        mode="markers+text", marker=dict(color=C["blue"], size=[max(d["commitment"] * 0.5,8) for d in deals], opacity=0.8),
    ...
    controls = card([
        ...
        html.Div([
            ...
            _field("Exit Price to NAV %", dcc.Input(id="disc-exit-price", type="number", value=100.0, step=0.5, style=INP)),
            _field("Hold Override (blank = deal hold)", dcc.Input(id="disc-hold-override", type="number", value=None, step=0.25, style=INP)),
...
def update_discount_return_output(nav_growth, exit_price_to_nav, hold_override, portfolio, pipeline):
    ...
    deals = [dict(d, pool="Portfolio") for d in (portfolio or [])] + [dict(d, pool="Pipeline") for d in (pipeline or [])]
    rows = []
    total_cost = total_exit = 0
    weighted_years = 0
    for d in deals:
        base_nav = float(d.get("pro_forma_nav") if d.get("status") == "Pipeline" and d.get("pro_forma_nav") is not None else d.get("current_nav", d.get("commitment", 0)) or 0)
    ...
    total_base_nav = sum(float(d.get("pro_forma_nav") if d.get("status") == "Pipeline" and d.get("pro_forma_nav") is not None else d.get("current_nav", d.get("commitment", 0)) or 0) for d in deals)
    ...
    summary = html.Div([
        kpi("Blended Entry Discount", f"{blended_discount:.1f}%" if blended_discount is not None else "—", "Versus base NAV", C["green"]),
        ...
        kpi("Blended Discount IRR", f"{portfolio_irr:.1f}%" if portfolio_irr is not None else "—", "Weighted by entry cost", C["purple"]),
...
def build_deal(nid, name, manager, deal_type, region, sector, commitment, moic, hold, deal_yield, spread, ltv,
    ...
    original_legal_commitment = float(original_legal_commitment if original_legal_commitment is not None else current_commitment)
    current_nav = float(current_nav if current_nav is not None else current_commitment)
    pro_forma_nav = float(pro_forma_nav) if pro_forma_nav is not None else (current_nav if status == "Pipeline" else None)
    return dict(
        id=nid, name=name, manager=manager or "", deal_type=deal_type, region=region or "Europe", sector=sector,
        commitment=current_commitment, current_commitment=current_commitment, original_legal_commitment=original_legal_commitment,
...
@app.callback(
    ...
    State("port-name", "value"), State("port-manager", "value"), State("port-type", "value"), State("port-region", "value"), State("port-sector", "value"),
    State("port-orig-commit", "value"), State("port-commit", "value"), State("port-moic", "value"), State("port-hold", "value"),
    State("port-yield", "value"), State("port-spread", "value"), State("port-ltv", "value"), State("port-price-to-nav", "value"), State("port-current-nav", "value"),
    State("port-security", "value"), State("port-rate", "value"),
    prevent_initial_call=True,
)
def add_portfolio(_, portfolio, nid, name, manager, deal_type, region, sector, original_legal_commitment, commitment, moic, hold, deal_yield, spread, ltv, price_to_nav, current_nav, security, rate_type):
    if not name:
        return portfolio, nid, "⚠ Enter a deal name."
    deal = build_deal(nid, name, manager, deal_type, region, sector, commitment, moic, hold, deal_yield, spread, ltv, original_legal_commitment, price_to_nav, current_nav, None, security, rate_type, status="Allocated")
...
@app.callback(
    ...
    State("pipe-name", "value"), State("pipe-manager", "value"), State("pipe-type", "value"), State("pipe-region", "value"), State("pipe-sector", "value"),
    State("pipe-stage", "value"), State("pipe-priority", "value"), State("pipe-orig-commit", "value"), State("pipe-commit", "value"),
    State("pipe-moic", "value"), State("pipe-hold", "value"), State("pipe-yield", "value"),
    State("pipe-spread", "value"), State("pipe-ltv", "value"), State("pipe-price-to-nav", "value"), State("pipe-current-nav", "value"), State("pipe-pro-forma-nav", "value"),
    prevent_initial_call=True,
)
def add_pipeline(_, pipeline, nid, name, manager, deal_type, region, sector, stage, priority, original_legal_commitment, commitment, moic, hold, deal_yield, spread, ltv, price_to_nav, current_nav, pro_forma_nav):
    if not name:
        return pipeline, nid, "⚠ Enter a deal name."
    deal = build_deal(nid, name, manager, deal_type, region, sector, commitment, moic, hold, deal_yield, spread, ltv, original_legal_commitment, price_to_nav, current_nav, pro_forma_nav, stage=stage, priority=priority, status="Pipeline")
