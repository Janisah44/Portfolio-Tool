...
def upload_fund_cf_file(contents, filename):
    ...
    try:
        ...
        for col in range(1, ws.max_column + 1):
            ...
            if value in template_labels.values():
                for key, label in template_labels.items():
                    if value == label:
                        block_starts[key] = col

        if block_starts.get('net_cf') and block_starts.get('nav'):
            print(f"Detected template structure: {block_starts}")
            block_order = ['net_cf', 'calls', 'distributions', 'nav']
            bounds = {}
            for idx, key in enumerate(block_order):
                start = block_starts.get(key)
                if not start:
                    continue
                next_starts = [block_starts[k] for k in block_order[idx + 1:] if k in block_starts]
                end = min(next_starts) - 1 if next_starts else ws.max_column - 1
                bounds[key] = (start, end)

            for row in range(7, ws.max_row + 1):
                deal_name = ws.cell(row, 5).value
                if not deal_name:
                    continue

                deal_name = str(deal_name).strip()
                if deal_name.lower().startswith('total ') or deal_name in ['Cash', 'Liquidity 1', 'Liquidity 2']:
                    continue

                raw_unfunded = ws.cell(row, 13).value
                raw_unfunded = float(raw_unfunded) if isinstance(raw_unfunded, (int, float)) else 0.0
                unfunded = raw_unfunded / 1_000_000 if abs(raw_unfunded) > 1000 else raw_unfunded

                deal_data = {
                    'name': deal_name,
                    'investments': ws.cell(row, 6).value or deal_name,
                    ...
                    'commitment': float(ws.cell(row, 11).value or 0),
                    'paid_in': float(ws.cell(row, 12).value or 0),
                    'unfunded': unfunded,
                ...
                for series_name, (start_col, end_col) in bounds.items():
                    for col in range(start_col, min(end_col, ws.max_column) + 1):
                        month_val = ws.cell(2, col).value
                        if not month_val:
                            continue
                        month_key = pd.to_datetime(month_val).strftime('%Y-%m-%d')
                        cell_val = ws.cell(row, col).value
                        num = float(cell_val) if isinstance(cell_val, (int, float)) else 0.0

                        if series_name == 'net_cf':
                            deal_data['net_cf_series'][month_key] = num
                        elif series_name == 'calls':
                            deal_data['calls_series'][month_key] = num
                        elif series_name == 'distributions':
                            deal_data['distributions_series'][month_key] = num
                        elif series_name == 'nav':
                            deal_data['nav_series'][month_key] = num

                ordered_months = sorted(deal_data['net_cf_series'].keys())
                deal_data['monthly_cfs'] = {idx: deal_data['net_cf_series'][mk] for idx, mk in enumerate(ordered_months)}

                latest_nav = get_latest_available_nav_for_deal([deal_data], deal_name)
                if latest_nav is None:
                    non_zero_navs = [v for _, v in sorted(deal_data['nav_series'].items()) if v not in (None, 0, 0.0)]
                    latest_nav = non_zero_navs[-1] if non_zero_navs else deal_data['commitment']

                deal_data['nav'] = float(latest_nav or 0)
                deal_data['current_commitment'] = float(deal_data['paid_in'] or max(deal_data['commitment'] - deal_data['unfunded'], 0))
            ...
            if header_row:
                ...
                for row in range(header_row + 1, min(header_row + 150, ws.max_row + 1)):
                    name_col = headers.get('AIC Name') or headers.get('Investments') or headers.get('Investment')
                    deal_name = ws.cell(row, name_col).value if name_col else None
                    if not deal_name or 'Total' in str(deal_name):
                        continue

                    commitment = ws.cell(row, headers.get('Commitment ($m)', 11)).value or 0
                    paid_in = ws.cell(row, headers.get('Paid In', 12)).value or 0
                    unfunded = ws.cell(row, headers.get('Unfunded', 13)).value or 0

                    deals_data.append({
                        'name': str(deal_name).strip(),
                        'investments': str(deal_name).strip(),
                        'portfolio_type': '',
                        'type': str(ws.cell(row, headers.get('Investment Type', 8)).value or ''),
                        'investment_status': str(ws.cell(row, headers.get('Investment Status', 9)).value or ''),
                        'commitment_year': ws.cell(row, headers.get('Commitment Year', 10)).value,
                        'commitment': float(commitment or 0),
                        'paid_in': float(paid_in or 0),
                        'unfunded': float(unfunded or 0),
                        'current_commitment': float(paid_in or 0),
                        'nav': float(paid_in or commitment or 0),
                        'monthly_cfs': {},
                        'net_cf_series': {},
                        'calls_series': {},
                        'distributions_series': {},
...
def generate_monthly_cf_table(deals, uploaded_data, num_months, cf_type):
    ...
    if uploaded_data:
        ...
        for deal in uploaded_data:
            for series_name in ['net_cf_series', 'calls_series', 'distributions_series', 'nav_series']:
                all_months.extend(list((deal.get(series_name) or {}).keys()))
        month_keys = sorted(set(all_months))[:num_months]
        month_labels = [pd.to_datetime(m).strftime('%b-%y') for m in month_keys]

        meta_cols = ['AIC Name', 'Investments', 'Portfolio Type', 'Investment Type', 'Investment Status',
                     'Commitment Year', 'Commitment ($m)', 'Paid In', 'Unfunded']

        def build_rows(series_name):
            rows = []
            for deal in uploaded_data:
                row = {
                    ...
                    'Commitment Year': deal.get('commitment_year', ''),
                    'Commitment ($m)': f"${float(deal.get('commitment', 0) or 0):.1f}M",
                    'Paid In': f"${float(deal.get('paid_in', deal.get('current_commitment', 0)) or 0):.1f}M",
                    'Unfunded': f"${float(deal.get('unfunded', 0) or 0):.1f}M",
                }
                series = deal.get(series_name) or {}
                for mk, label in zip(month_keys, month_labels):
                    val = series.get(mk, 0)
                    row[label] = f"${float(val):.2f}M" if val not in (None, 0, 0.0, '') else "-"
                rows.append(row)
            return rows, meta_cols + month_labels

        sections = []
        if cf_type in ['all', 'net']:
            rows, cols = build_rows('net_cf_series')
            sections.append(build_cashflow_template_table("Net CF", rows, cols))
        if cf_type == 'all':
            rows, cols = build_rows('calls_series')
            sections.append(build_cashflow_template_table("Capital Calls", rows, cols))
            rows, cols = build_rows('distributions_series')
            sections.append(build_cashflow_template_table("Distributions", rows, cols))
        if cf_type in ['all', 'nav']:
            rows, cols = build_rows('nav_series')
            sections.append(build_cashflow_template_table("NAV", rows, cols))
...
@app.callback(
    ...
    [Input('deals-store', 'data'), Input('overview-exposure-table', 'data'),
     Input('fund-cf-data-store', 'data')]
)
def update_exposure_comparison(deals, exposure_rows, fund_cf_data):
    deals_enriched = enrich_deals_with_latest_nav(deals, fund_cf_data)
    total_nav = sum(d.get('nav', d.get('size', 0)) for d in (deals_enriched or []))

    if not exposure_rows or total_nav == 0:
        return dbc.Alert("Add deals and save targets to see exposure comparison.", color="info")

    rows = []
    for row in exposure_rows:
        dim = row.get('dimension', '')
        cat = row.get('category', '')
        target_pct = float(row.get('target_pct', 0))
        min_pct = float(row.get('min_pct', 0))
        max_pct = float(row.get('max_pct', 0))

        # Calculate current exposure
        if dim == 'Strategy':
            current_nav = sum(d.get('nav', d.get('size', 0)) for d in (deals_enriched or []) if d.get('strategy') == cat)
        elif dim == 'Region':
            current_nav = sum(d.get('nav', d.get('size', 0)) for d in (deals_enriched or []) if d.get('geography') == cat)
        elif dim == 'Sector':
            current_nav = sum(d.get('nav', d.get('size', 0)) for d in (deals_enriched or []) if d.get('sector') == cat)
        elif dim == 'Vintage':
            current_nav = sum(d.get('nav', d.get('size', 0)) for d in (deals_enriched or []) if str(d.get('vintage', '')) == str(cat))
        else:
            current_nav = 0

        current_pct = (current_nav / total_nav * 100) if total_nav > 0 else 0
        ...
        rows.append(html.Tr([
            html.Td(dim, style={'fontFamily': C['mono'], 'fontSize': '12px'}),
            html.Td(cat, style={'fontFamily': C['mono'], 'fontSize': '12px', 'fontWeight': 'bold'}),
            html.Td(f"{current_pct:.1f}%", style={'fontFamily': C['mono'], 'color': C['blue']}),
            html.Td(f"{target_pct:.1f}%", style={'fontFamily': C['mono'], 'color': C['muted']}),
            html.Td(f"{min_pct:.1f}% – {max_pct:.1f}%", style={'fontFamily': C['mono'], 'color': C['muted'], 'fontSize': '11px'}),
            html.Td(status, style={'color': status_color, 'fontWeight': 'bold', 'fontSize': '12px'}),
        ]))

    return dbc.Table([
        html.Thead(html.Tr([
            html.Th('Dimension'), html.Th('Category'), html.Th('Current %'),
            html.Th('Target %'), html.Th('Range'), html.Th('Status')
