...
def tab_analytics(portfolio, pipeline, config):
    ...
    def bar(lbls, vals, color, title):
        ...
        f.update_layout(**CHART_BASE, margin=dict(l=52,r=20,t=40,b=40), xaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), yaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), legend=dict(bgcolor=C["panel"],bordercolor=C["border"],borderwidth=1), height=300, title=title)
        return f
    ...
    fig_conc.update_layout(**CHART_BASE, margin=dict(l=52,r=20,t=40,b=40), xaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), yaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), legend=dict(bgcolor=C["panel"],bordercolor=C["border"],borderwidth=1), height=300, title="Concentration Risk (% of NAV)")
    ...
    fig_scatter.update_layout(**CHART_BASE, margin=dict(l=52,r=20,t=40,b=40), xaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), yaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), legend=dict(bgcolor=C["panel"],bordercolor=C["border"],borderwidth=1), height=340, title="IRR vs NAV — Deal-level Scatter",
...
def tab_segments(portfolio, pipeline, placeholders, config):
    ...
    fig_alloc.update_layout(**CHART_BASE, margin=dict(l=52,r=20,t=40,b=40), xaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), yaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), legend=dict(bgcolor=C["panel"],bordercolor=C["border"],borderwidth=1), height=300, title="Segment Allocation")
    ...
    fig_contrib.update_layout(**CHART_BASE, margin=dict(l=52,r=20,t=40,b=40), xaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), yaxis=dict(gridcolor=C["border"],zerolinecolor=C["border"]), legend=dict(bgcolor=C["panel"],bordercolor=C["border"],borderwidth=1), height=300, title="TWR Contribution (NAV × IRR)")
...
def populate_edit(btn_click, edit_idx_store, selected, portfolio):
    ctx = callback_context
    if not portfolio:
        return [no_update]*20
    triggered = ctx.triggered[0]["prop_id"] if ctx.triggered else ""
    # When triggered by edit-idx store (from proforma routing), use store value directly
    if "edit-idx" in triggered and edit_idx_store is not None:
        idx = edit_idx_store
    elif selected:
        idx = selected[0]
    else:
...
def add_pipeline(_, pipeline, nid, edit_pipe_idx, name, strategy, stage_deal, sector,
    ...
    if not name: return no_update, no_update, no_update, "⚠ Enter a deal name."
    p = pipeline or []
    ...
    if edit_pipe_idx is not None and 0 <= edit_pipe_idx < len(p):
        ...
        return p, nid, None, f"✓ Updated: {name}"
    else:
        deal["id"] = nid
        return p+[deal], nid+1, None, f"✓ Added pipeline: {name}"
...
@app.callback(
    ...
    Input("edit-pipe-idx","data"),
    State("pipeline-table","selected_rows"), State("pipe-store","data"),
    prevent_initial_call=True,
)
def populate_pipe_edit(btn_click, edit_pipe_idx_store, selected, pipeline):
    ctx = callback_context
    if not pipeline:
        return [no_update]*11
    triggered = ctx.triggered[0]["prop_id"] if ctx.triggered else ""
    if "edit-pipe-idx" in triggered and edit_pipe_idx_store is not None:
        idx = edit_pipe_idx_store
    elif selected:
        idx = selected[0]
    else:
        return [no_update]*11
...
def add_placeholder(_, placeholders, nid, edit_ph_idx, name, strategy, dtype, region, size, irr, month):
    if not name: return no_update, no_update, no_update, "⚠ Enter a name."
    p = placeholders or []
    ...
    if edit_ph_idx is not None and 0 <= edit_ph_idx < len(p):
        ...
        ph["id"] = nid
        return p+[ph], nid+1, None, f"✓ Added: {name}"
