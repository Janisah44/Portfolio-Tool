...
class DataFrameWorksheetAdapter:
    """Tiny adapter so pandas-loaded sheets can be accessed like openpyxl worksheets."""

    def __init__(self, df):
        self.df = df.fillna(value=np.nan)
        self.max_row = int(df.shape[0])
        self.max_column = int(df.shape[1])

    def cell(self, row, col):
        try:
            value = self.df.iat[row - 1, col - 1]
        except Exception:
            value = None
        if pd.isna(value):
            value = None
...
def load_excel_sheet(filename, decoded_bytes, preferred_sheet_names=None):
    """Load an Excel sheet from uploaded bytes with support for .xlsx, .xlsm and .xlsb."""
    preferred_sheet_names = preferred_sheet_names or []
    fname = (filename or '').lower()
    bio = BytesIO(decoded_bytes)

    if fname.endswith('.xlsb'):
        engine = 'pyxlsb'
        xls = pd.ExcelFile(bio, engine=engine)
        sheet_name = next((s for s in preferred_sheet_names if s in xls.sheet_names), xls.sheet_names[0])
        df = pd.read_excel(BytesIO(decoded_bytes), sheet_name=sheet_name, header=None, engine=engine)
        return DataFrameWorksheetAdapter(df), sheet_name

    from openpyxl import load_workbook
    wb = load_workbook(bio, data_only=True)
    sheet_name = next((s for s in preferred_sheet_names if s in wb.sheetnames), wb.sheetnames[0])
            ...
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        ...
                        html.Small("Upload the 'Fund Level CF' tab from your Excel file (.xlsx, .xlsm, .xlsb)",
            ...
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        ...
                        html.Small("Upload the 'Liquidity Pull' tab from your Excel file (.xlsx, .xlsm, .xlsb)",
                                   className="text-muted d-block mt-2")
                    ], width=6),
                    dbc.Col([
...
def upload_fund_cf_file(contents, filename):
    ...
    try:
        ...
        ws, selected_sheet = load_excel_sheet(filename, decoded, target_sheets)
        print(f"Using Fund CF sheet: {selected_sheet}")
...
def generate_monthly_cf_chart(deals, uploaded_data, num_months):
    """Generate stacked monthly cashflow totals chart: Calls (red bars), Distributions (green bars), NAV (blue line)."""
    ...
    if uploaded_data:
        all_months_ordered = (
            uploaded_data[0].get('_all_month_keys') or
            sorted({mk for d in uploaded_data for mk in (d.get('calls_series') or {}).keys()})
        )
        display_months = all_months_ordered[:num_months]
        month_labels   = [pd.to_datetime(m).strftime("%b %Y") for m in display_months]

        calls = [sum(float((d.get('calls_series') or {}).get(mk, 0) or 0) for d in uploaded_data)
                 for mk in display_months]
        dists = [sum(float((d.get('distributions_series') or {}).get(mk, 0) or 0) for d in uploaded_data)
                 for mk in display_months]
        nav   = [sum(float((d.get('nav_series') or {}).get(mk, 0) or 0) for d in uploaded_data)
                 for mk in display_months]
    else:
        base_date = datetime(2026, 1, 1)
        month_labels = []
        ...
        for i in range(num_months):
            month_labels.append((base_date + relativedelta(months=i)).strftime("%b %Y"))
            calls.append(0)
            dists.append(0)
            nav.append(total_nav)

    fig.add_trace(go.Bar(
        x=month_labels, y=calls, name='Called Capital',
        marker_color=C['red'], opacity=0.8,
        hovertemplate='%{x}<br>Calls: $%{y:.2f}M<extra></extra>'
    ))
    fig.add_trace(go.Bar(
        x=month_labels, y=dists, name='Distributions',
        marker_color=C['green'], opacity=0.8,
        hovertemplate='%{x}<br>Distributions: $%{y:.2f}M<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
        x=month_labels, y=nav, name='Ending NAV',
        line=dict(color=C['blue'], width=3),
    ...
    fig.update_layout(
        **CHART_BASE,
        barmode='group',
        height=420,
        yaxis =dict(title='Cashflows ($mm)', gridcolor=C['border'], zeroline=True, zerolinecolor=C['border2']),
        yaxis2=dict(title='NAV ($mm)', overlaying='y', side='right', gridcolor=C['border']),
        hovermode='x unified',
        legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1)
    )
    return fig
...
def upload_liquidity_file(contents, filename):
    ...
    try:
        ...
        ws, selected_sheet = load_excel_sheet(filename, decoded, sheet_names)
        print(f"Using Liquidity sheet: {selected_sheet}")
