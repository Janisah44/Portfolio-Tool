...
def run_mc_simulation(n_sims, n_quarters, base_yield, yield_vol, default_rate_annual,
    ...
    TWR per period uses the cash-flow-adjusted formula:
        Period Return = (Ending NAV + Distributions - Capital Calls) / Beginning NAV - 1

    Where:
        Beginning NAV   = NAV at start of quarter (before any cash flows)
        Distributions   = interest income paid out to investors + any net losses (negative)
        Capital Calls   = recycled principal reinvested into the portfolio this quarter
        Ending NAV      = NAV after defaults and repayments (before recycling injection)

    This correctly strips out the effect of external cash flows so that TWR
    measures manager skill / portfolio performance, not the timing of capital deployment.
    """
    random.seed(42)
    np.random.seed(42)
    ...
    for _ in range(n_sims):
        ...
        for q in range(n_quarters):
            nav_begin = nav  # Beginning NAV — denominator of TWR formula

            # ── Stochastic yield shock ────────────────────────────────────────
            ...
            # ── Interest income distributed to investors (external outflow) ───
            gross_interest = nav * q_yield
            fees = nav * q_fee
            net_interest_distributed = gross_interest - fees  # net of fee drag; flows OUT to investors

            # ── NAV after credit losses ───────────────────────────────────────
            nav_after_loss = max(nav - loss, 0)

            # ── Principal repayment and recycling ─────────────────────────────
            q_hold_quarters = max(int(round(wa_hold * 4)), 1)
            repayment = nav_after_loss / q_hold_quarters if q > 4 else 0

            # Ending NAV: after losses and repayments, BEFORE recycling injection
            # This is the "true" portfolio value before new capital comes in
            nav_end = max(nav_after_loss - repayment, 0)

            # Recycled principal = capital call back INTO the fund (external inflow)
            capital_call = repayment * recycling_rate

            # ── Correct TWR period return formula ─────────────────────────────
            # Period Return = (Ending NAV + Distributions - Capital Calls) / Beginning NAV - 1
            #
            # Distributions = net_interest_distributed (paid out to investors)
            # Capital Calls  = capital_call (recycled principal reinvested)
            # Ending NAV     = nav_end (after losses, before recycling injection)
            #
            # Intuition: we add back distributions because they left the portfolio
            # (but represent real return), and subtract capital calls because they
            # inflated ending NAV without being a return.
            numerator = nav_end + net_interest_distributed - capital_call
            q_return = numerator / max(nav_begin, 1e-9) - 1

            # ── Update NAV for next quarter (inject recycled principal) ───────
            nav = nav_end + capital_call
...
def run_montecarlo(_, n_sims, n_quarters, base_yield, yield_vol, default_rate, recovery,
    ...
    note = html.Div(
        "TWR formula: Period Return = (Ending NAV + Distributions − Capital Calls) / Beginning NAV − 1. "
        "Distributions = net interest paid to investors each quarter. Capital Calls = recycled principal reinvested. "
        "Ending NAV is measured after credit losses and repayments, before recycling injection. "
        "Defaults are exponentially distributed; yield shocks are normally distributed; fees deducted from interest before distribution. "
        "TWR eliminates the distortion of external cash flows, measuring portfolio performance independently of capital deployment timing.",
