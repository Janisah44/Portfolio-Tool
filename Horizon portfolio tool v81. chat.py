...
def chart_layout(**overrides):
    """Merge Plotly layout dicts safely to avoid duplicate kwargs like margin/xaxis/legend."""
    base = dict(CHART)
    merged = {}
...
app.layout = html.Div([
    ...
    dcc.Store(id="pipe-edit-idx",    data=None),
    dcc.Store(id="ph-edit-idx",      data=None),
...
def route(tab, portfolio, pipeline, placeholders, config, cf_data, liq_data, disc_store):
    ...
    if tab == "proforma":   return tab_proforma(p, pi, config)
    if tab == "deploy":     return tab_deploy(ph, config)
    if tab == "analytics":  return tab_analytics(p, pi, config)
    if tab == "segments":   return tab_segments(p, pi, ph, config)
    if tab == "cashflows":  return tab_cashflows(p, cf_data, disc_store)
    if tab == "liquidity":  return tab_liquidity(liq_data)
...
def tab_portfolio(portfolio, config):
    ...
    fig.update_layout(**chart_layout( height=280, margin=dict(t=50,b=10,l=10,r=10)))
    ...
    actions = card([
        ...
        html.Button("🔄 Pull Latest NAVs", id="sync-port-nav-btn", style={**BTN(C["teal"]), "width":"100%", "marginBottom":8}),
...
def tab_pipeline(pipeline, portfolio, config):
    ...
    fig_funnel.update_layout(**chart_layout( height=300, title="Pipeline Funnel ($M)"))
    ...
    form = html.Div([
        ...
        _form_row(
            ...
            html.Div([html.Label(" ",style=dict(display="block",marginBottom=4,fontSize=9)),
                      html.Button("+ Add / Save",id="add-pipe-btn",style={**BTN(C["purple"]),"minWidth":110})]),
            html.Div([html.Label(" ",style=dict(display="block",marginBottom=4,fontSize=9)),
                      html.Button("Clear",id="clear-pipe-btn",style={**BTN(C["surface"], C["muted"]),"border":f"1px solid {C['border2']}","minWidth":70})]),
        ),
...
def tab_proforma(portfolio, pipeline, config):
    ...
    # Combined table: current deals + pipeline (editable)
    total = all_nav or 1
    rows = []
    for i, d in enumerate(portfolio):
        rows.append({
            "_idx": i, "_source_key": "portfolio",
            "Source":"✅ Current","Deal":d["name"],"Manager":d.get("manager",""),
            "Strategy":d.get("strategy",""),"Type":d.get("deal_type",""),
            "Region":d.get("region",""),
            "NAV ($M)":round(float(d.get("nav",0) or 0),2),
            "Weight":f"{d.get('nav',0)/total*100:.1f}%",
            "IRR %":round(float(d.get('target_irr',0) or 0)*100,2),
            "MOIC":round(float(d.get('moic',0) or 0),2),
            "Vintage":d.get("vintage",""),
            "Status":d.get("allocation_status",""),
        })
    for i, p in enumerate(pipeline):
        rows.append({
            "_idx": i, "_source_key": "pipeline",
            "Source":"⏳ Pipeline","Deal":p["name"],"Manager":"",
            "Strategy":p.get("strategy",""),"Type":"Secondary",
            "Region":p.get("region",""),
            "NAV ($M)":round(float(p.get("size",0) or 0),2),
            "Weight":f"{p.get('size',0)/total*100:.1f}%",
            "IRR %":round(float(p.get('target_irr',0) or 0)*100,2),
            "MOIC":None,"Vintage":"",
            "Status":p.get("pipeline_stage",""),
        })
    ...
    tbl = dash_table.DataTable(
        ...
        columns=[
            ...
            {"name":"Deal","id":"Deal","editable":True},
            {"name":"Manager","id":"Manager","editable":True},
            {"name":"Strategy","id":"Strategy","editable":True},
            ...
            {"name":"IRR %","id":"IRR %","editable":True,"type":"numeric"},
            {"name":"MOIC","id":"MOIC","editable":True,"type":"numeric"},
            {"name":"Vintage","id":"Vintage","editable":True},
            {"name":"Status","id":"Status","editable":True},
            {"name":"_idx","id":"_idx","editable":False},
            {"name":"_source_key","id":"_source_key","editable":False},
        ] if rows else [],
        hidden_columns=["_idx","_source_key"],
        editable=True,
        style_cell=TBL_CELL, style_header=TBL_HEAD,
        style_data_conditional=TBL_ODD+[
            {"if":{"filter_query":'{Source} contains "Pipeline"'},"color":C["purple"],"fontStyle":"italic"},
            {"if":{"column_id":"IRR %"},"color":C["green"]},
            {"if":{"column_id":"NAV ($M)"},"color":C["sky"]},
        ],
        sort_action="native", filter_action="native",
    ...
    return html.Div([
        html.Div([
            ...
            html.Span("  (editable — changes save back to Portfolio or Pipeline)",
        ...
        card([section_lbl("Pro Forma Full Deal List"), tbl]),
    ])
...
def tab_deploy(placeholders, config):
    ...
    fig.update_layout(**chart_layout( height=300, title="Planned Commitment Schedule ($M)"))

    form = html.Div([
        section_lbl("Add Planned Future Commitment"),
        html.Div([
            ...
            html.Div([html.Label(" ",style=dict(display="block",marginBottom=4,fontSize=9)),
                      html.Button("+ Add / Save",id="add-ph-btn",style={**BTN(C["teal"]),"minWidth":110})]),
            html.Div([html.Label(" ",style=dict(display="block",marginBottom=4,fontSize=9)),
                      html.Button("Clear",id="clear-ph-btn",style={**BTN(C["surface"], C["muted"]),"border":f"1px solid {C['border2']}","minWidth":70})]),
...
def tab_cashflows(portfolio, cf_data, disc_store):
    ...
    if disc_rows:
        ...
        fig_disc = go.Figure()
        fig_disc.update_layout(**chart_layout( height=200))
...
def tab_liquidity(liq_data):
    ...
    if nav_proj:
        ...
        fig_nav.update_layout(**chart_layout( height=340, title="NAV End Projections",
                               yaxis_title="NAV ($M)", hovermode="x unified"))
    else:
        fig_nav = go.Figure()
        fig_nav.update_layout(**chart_layout( height=200, title="NAV Projections (no data)"))
...
def tab_analytics(portfolio, pipeline, config):
    ...
    def bar(lbls, vals, color, title):
        ...
        f.update_layout(**chart_layout( height=300, title=title))
        return f
    ...
    def pie(lbls, vals, colors, title):
        ...
        f.update_layout(**chart_layout( height=300, title=title, margin=dict(t=50,b=10,l=10,r=10)))
        return f
    ...
    fig_conc.update_layout(**chart_layout( height=300, title="Concentration Risk (% of NAV)"))
    ...
    fig_scatter.update_layout(**chart_layout( height=340, title="IRR vs NAV — Deal-level Scatter",
...
def tab_segments(portfolio, pipeline, placeholders, config):
    ...
    fig_twr.update_layout(**chart_layout( height=360, title="12-Month Cumulative TWR Forecast by Segment",
    ...
    fig_alloc.update_layout(**chart_layout( height=300, title="Segment Allocation"))
    ...
    fig_contrib.update_layout(**chart_layout( height=300, title="TWR Contribution (NAV × IRR)"))
...
def tab_drypowder(portfolio, placeholders, config):
    ...
    fig.update_layout(**chart_layout( height=400, title="12-Month Dry Powder Forecast",
...
def tab_pacing(portfolio, pipeline, placeholders, config):
    ...
    fig_dep.update_layout(**chart_layout( height=340, barmode="stack", title="Capital Deployment & Distributions"))
    ...
    fig_nav.update_layout(**chart_layout( height=340, title="NAV & Dry Powder Projection"))
    ...
    fig_util = go.Figure(go.Bar(x=df["Quarter"],y=df["Utilisation %"],marker_color=C["purple"],
                                 ...
                                 textfont_color=C["text"]))
    fig_util.update_layout(**chart_layout( height=300, title="Fund Utilisation Path", yaxis_title="%"))
...
def tab_returns(portfolio, pipeline, config):
    ...
    fig_moic.update_layout(**chart_layout( height=300, title="Required MOIC by Hold Period",
    ...
    fig_scatter.update_layout(**chart_layout( height=340, title="Deal IRR vs Hold Period",
...
def add_or_update_portfolio(_, portfolio, nid, edit_idx,
                             ...
                             hold, moic, p2nav, vintage, segment, alloc_status):
    if not name:
...
@app.callback(
    ...
    State("pipe-store","data"), State("next-id","data"), State("pipe-edit-idx","data"),
...
def add_pipeline(_, pipeline, nid, edit_idx, name, strategy, stage_deal, sector,
    ...
    if edit_idx is not None and 0 <= edit_idx < len(pipe):
        deal["id"] = pipe[edit_idx]["id"]
        deal["date_added"] = pipe[edit_idx].get("date_added", deal["date_added"])
        pipe[edit_idx] = deal
        return pipe, nid, f"✓ Updated pipeline: {name}"
    return pipe+[deal], nid+1, f"✓ Added pipeline: {name}"


# ── Pipeline: populate edit form ─────────────────────────────────────────────
@app.callback(
    Output("pipe-edit-idx","data"),
    ...
    Output("pipe-priority","value"), Output("pipe-msg","children", allow_duplicate=True),
    Input("edit-pipe-btn","n_clicks"),
    State("pipeline-table","selected_rows"), State("pipe-store","data"),
    prevent_initial_call=True,
)
def populate_pipeline_edit(_, selected, pipeline):
    if not selected or not pipeline:
        return (no_update,)*10 + (no_update,)
    idx = selected[0]
    if idx >= len(pipeline):
        return (no_update,)*10 + (no_update,)
    d = pipeline[idx]
    return (
        idx, d.get("name",""), d.get("strategy",STRATEGIES[0]), d.get("stage_deal","Buyout"),
        d.get("sector","Technology"), d.get("region","North America"), d.get("size",0),
        round(float(d.get("target_irr",0))*100,2), d.get("pipeline_stage","Screening"),
        d.get("priority","Medium"),
        f"📝 Editing pipeline: {d.get('name','')} — click '+ Add / Save' to update."
    )


# ── Pipeline: clear form ─────────────────────────────────────────────────────
@app.callback(
    Output("pipe-edit-idx","data", allow_duplicate=True),
    Output("pipe-name","value", allow_duplicate=True),
    Output("pipe-msg","children", allow_duplicate=True),
    Input("clear-pipe-btn","n_clicks"),
    prevent_initial_call=True,
)
def clear_pipeline_form(_):
    return None, "", ""
...
@app.callback(
    ...
    State("ph-store","data"), State("next-id","data"), State("ph-edit-idx","data"),
...
def add_placeholder(_, placeholders, nid, edit_idx, name, strategy, dtype, region, size, irr, month):
    ...
    if edit_idx is not None and 0 <= edit_idx < len(phs):
        ph["id"] = phs[edit_idx]["id"]
        ph["date_added"] = phs[edit_idx].get("date_added", ph["date_added"])
        phs[edit_idx] = ph
        return phs, nid, f"✓ Updated: {name}"
    return phs+[ph], nid+1, f"✓ Added: {name}"


# ── Placeholder: populate edit form ──────────────────────────────────────────
@app.callback(
    Output("ph-edit-idx","data"),
    ...
    Output("ph-month","value"), Output("ph-msg","children", allow_duplicate=True),
    Input("edit-ph-btn","n_clicks"),
    State("ph-table","selected_rows"), State("ph-store","data"),
    prevent_initial_call=True,
)
def populate_placeholder_edit(_, selected, placeholders):
    if not selected or not placeholders:
    ...
    d = placeholders[idx]
    return (
        idx, d.get("name",""), d.get("strategy",STRATEGIES[0]), d.get("deal_type","Secondary"),
        d.get("region","North America"), d.get("size",0), round(float(d.get("target_irr",0))*100,2),
        d.get("expected_month",3),
        f"📝 Editing future deployment: {d.get('name','')} — click '+ Add / Save' to update."
    )


# ── Placeholder: clear form ──────────────────────────────────────────────────
@app.callback(
    Output("ph-edit-idx","data", allow_duplicate=True),
    Output("ph-name","value", allow_duplicate=True),
    Output("ph-msg","children", allow_duplicate=True),
    Input("clear-ph-btn","n_clicks"),
    prevent_initial_call=True,
)
def clear_placeholder_form(_):
    return None, "", ""
...
def run_mc(_, portfolio, config, mean_pct, std_pct, n_sims):
    ...
    fig_dist.update_layout(**chart_layout( height=320, title=f"Net TWR Distribution ({n_sims:,} simulations)",
    ...
    fig_sens.add_hline(y=tgt*100, line_dash="dash", line_color=C["red"],
                       annotation_text=f"Target {tgt:.1%}")
    fig_sens.update_layout(**chart_layout( height=300, title="Net TWR Sensitivity to Future Deal IRR",
...
# ── Portfolio: sync NAVs from latest uploaded Fund Level CF ──────────────────
@app.callback(
    Output("port-store","data", allow_duplicate=True),
    Output("port-action-msg","children", allow_duplicate=True),
    Input("sync-port-nav-btn","n_clicks"),
    State("port-store","data"), State("fund-cf-store","data"),
    prevent_initial_call=True,
)
def sync_portfolio_navs(_, portfolio, cf_data):
    if not portfolio:
        return no_update, "No portfolio deals to update."
    if not cf_data:
        return no_update, "Upload Fund Level CF first."
    updated = []
    count = 0
    latest_month = None
    all_months = cf_data[0].get("_all_month_keys") if cf_data else None
    if all_months:
        latest_month = pd.to_datetime(all_months[-1]).strftime("%b %Y")
    for d in portfolio:
        nd = dict(d)
        matched = None
        name_key = str(d.get("name","")).strip().lower()
        for cf in cf_data:
            if str(cf.get("name","")).strip().lower() == name_key:
                matched = cf
                break
        if matched:
            latest_nav = _latest_nav(cf_data, d.get("name"))
            if latest_nav not in (None, 0):
                nd["nav"] = float(latest_nav)
                nd["current_commitment"] = float(matched.get("current_commitment", nd.get("current_commitment", 0)) or nd.get("current_commitment", 0))
                if matched.get("commitment") not in (None, 0):
                    nd["total_commitment"] = float(matched.get("commitment"))
                count += 1
        updated.append(nd)
    msg = f"🔄 Updated {count} deal NAV(s)" + (f" from latest month {latest_month}" if latest_month else "")
    return updated, msg


# ── Pro forma inline edits sync back to stores ───────────────────────────────
@app.callback(
    Output("port-store","data", allow_duplicate=True),
    Output("pipe-store","data", allow_duplicate=True),
    Input("proforma-table","data_timestamp"),
    State("proforma-table","data"),
    State("port-store","data"), State("pipe-store","data"),
    prevent_initial_call=True,
)
def sync_proforma_edits(_, rows, portfolio, pipeline):
    if not rows:
        return no_update, no_update
    port = [dict(x) for x in (portfolio or [])]
    pipe = [dict(x) for x in (pipeline or [])]
    changed_port = False
    changed_pipe = False
    for r in rows:
        ...
        try:
            ...
            continue
        if src == "portfolio" and 0 <= idx < len(port):
            d = port[idx]
            d["name"] = r.get("Deal", d.get("name",""))
            d["manager"] = r.get("Manager", d.get("manager",""))
            d["strategy"] = r.get("Strategy", d.get("strategy",""))
            d["deal_type"] = r.get("Type", d.get("deal_type",""))
            d["region"] = r.get("Region", d.get("region",""))
            try: d["nav"] = float(r.get("NAV ($M)", d.get("nav",0)) or 0)
            except: pass
            try: d["target_irr"] = float(r.get("IRR %", d.get("target_irr",0)*100) or 0)/100
            except: pass
            try: d["moic"] = float(r.get("MOIC", d.get("moic",0)) or 0)
            except: pass
            try: d["vintage"] = int(float(r.get("Vintage", d.get("vintage",0)) or 0)) if str(r.get("Vintage","")).strip() != "" else d.get("vintage")
            except: pass
            d["allocation_status"] = r.get("Status", d.get("allocation_status",""))
            changed_port = True
        elif src == "pipeline" and 0 <= idx < len(pipe):
            d = pipe[idx]
            d["name"] = r.get("Deal", d.get("name",""))
            d["strategy"] = r.get("Strategy", d.get("strategy",""))
            d["region"] = r.get("Region", d.get("region",""))
            try: d["size"] = float(r.get("NAV ($M)", d.get("size",0)) or 0)
            except: pass
            try: d["target_irr"] = float(r.get("IRR %", d.get("target_irr",0)*100) or 0)/100
            except: pass
            d["pipeline_stage"] = r.get("Status", d.get("pipeline_stage",""))
            changed_pipe = True
    return (port if changed_port else no_update, pipe if changed_pipe else no_update)
