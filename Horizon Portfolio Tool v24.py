"""
Horizon Portfolio Tool - INSTITUTIONAL GRADE
...
Run: python Horizon_Portfolio_Tool.py
        ...
        dbc.Modal([
            ...
            dbc.ModalBody([
                dbc.Alert([
                    ...
                    "Enter ACTUAL NAV of deals already in the portfolio. These do NOT reduce dry powder. Dry powder is only for pipeline/future deals."
                ...
                dbc.Row([
                    dbc.Col([
                        ...
                        html.Small("Total committed capital to this investment", className="text-muted")
                    ], width=4),
                    dbc.Col([
                        dbc.Label("Current NAV ($mm) *", style={'fontWeight': 'bold'}),
                        dbc.Input(id="in-size", type="number", step=0.1, placeholder="35.0"),
                        html.Small("Current Net Asset Value (mark-to-market)", className="text-muted")
                    ], width=4),
                    dbc.Col([
                        ...
                        html.Small("Auto-calculated: Commitment - NAV", className="text-muted")
                    ], width=4),
                ], className="mb-3"),
                dbc.Row([
                    dbc.Col([
                ...
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Select(id="in-vintage", options=[
                            {"label": str(y), "value": y} for y in range(2026, 2015, -1)
                        ], value=2024)
                    ], width=3),
                    dbc.Col([
                        ...
                        html.Small("Categorize for TWR tracking", className="text-muted")
                    ], width=3),
                    dbc.Col([
                            ...
                            dbc.Col([
                                dbc.RadioItems(
                                    ...
                                    options=[
                                        ...
                                        {"label": " Pro Forma (Current + Pipeline + Future)", "value": "proforma"},
                                    ],
                                    value="current",
                                    inline=True,
                                    style={'fontFamily': C['sans'], 'fontSize': '14px'}
                                )
                            ], width=9)
                        ])
                    ])
...
@app.callback(
    ...
    [State('in-name', 'value'), State('in-strategy', 'value'),
     State('in-commitment', 'value'), State('in-size', 'value'),
     State('in-irr', 'value'), State('in-hold', 'value'), State('in-vintage', 'value'),
     State('in-segment', 'value'), State('in-sector', 'value'), State('in-geo', 'value'),
     State('deals-store', 'data'), State('config-store', 'data')],
    prevent_initial_call=True
)
def add_deal(n, name, strat, commitment, size, irr, hold, vint, segment, sec, geo, deals, config):
    if not all([name, strat, commitment, size, irr]):
        return deals
...
def update_portfolio_table(deals):
    ...
    for idx, d in enumerate(deals):
        ...
        rows.append(dbc.Card([dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    html.H5(d['name'], className="mb-1", style={'fontFamily': C['sans'], 'color': C['text']}),
                    html.Small(f"{d['strategy']} • {d['sector']} • {d['geography']}",
                               className="text-muted", style={'fontFamily': C['mono']})
                ], width=4),
                dbc.Col([
                    html.Div([
                        html.Strong("Commitment: ", style={'fontFamily': C['sans'], 'color': C['muted']}),
                        html.Span(f"${commitment:.1f}M", style={'fontFamily': C['mono'], 'color': C['blue']}),
                        html.Br(),
                        html.Strong("Current NAV: ", style={'fontFamily': C['sans'], 'color': C['muted']}),
                        html.Span(f"${nav:.1f}M", style={'fontFamily': C['mono'], 'color': C['green']}),
                        html.Br(),
                        html.Strong("Unfunded: ", style={'fontFamily': C['sans'], 'color': C['muted']}),
                        html.Span(f"${unfunded:.1f}M", style={'fontFamily': C['mono'], 'color': C['amber']})
                    ], style={'fontSize': '13px'})
                ], width=3),
                dbc.Col([
                    html.Div([
                        html.Strong("IRR: ", style={'fontFamily': C['sans'], 'color': C['muted']}),
                        html.Span(f"{d['target_irr']:.1%}", style={'fontFamily': C['mono'], 'color': C['text']}),
                        html.Br(),
                        html.Strong("MOIC: ", style={'fontFamily': C['sans'], 'color': C['muted']}),
                        html.Span(f"{d.get('moic', 0):.2f}x", style={'fontFamily': C['mono'], 'color': C['text']})
                    ], style={'fontSize': '13px'})
                ], width=2),
                dbc.Col([
                    html.Div([
                        html.Strong("Hold: ", style={'fontFamily': C['sans'], 'color': C['muted']}),
                        html.Span(f"{d.get('hold_period', 5):.1f}y",
                                  style={'fontFamily': C['mono'], 'color': C['text']}),
                        html.Br(),
                        html.Strong("Vintage: ", style={'fontFamily': C['sans'], 'color': C['muted']}),
                        html.Span(str(d.get('vintage', 2024)), style={'fontFamily': C['mono'], 'color': C['text']}),
                        html.Br(),
                        html.Strong("Segment: ", style={'fontFamily': C['sans'], 'color': C['muted']}),
...
def upload_fund_cf_file(contents, filename):
    """Parse uploaded Fund Level CF Excel file"""
    if contents is None:
        return None, ""
    ...
    try:
        ...
        sheet_names = ['Fund Level CF', 'Fund_Level_CF', 'FundLevelCF']
        ws = None
        ...
        # Parse the sheet - extract deals and monthly data
        deals_data = []

        # Find header row (look for "AIC Name" or "Investment")
        header_row = None
        for row in range(1, min(20, ws.max_row + 1)):
            for col in range(1, min(10, ws.max_column + 1)):
                val = ws.cell(row, col).value
                if val and ('AIC Name' in str(val) or 'Investment' in str(val)):
                    header_row = row
                    break
            if header_row:
                break
        ...
        if header_row:
            ...
            for row in range(header_row + 1, min(header_row + 50, ws.max_row + 1)):
                deal_name = ws.cell(row, 6).value  # Column F typically has investment name
                if deal_name and str(deal_name).strip() and 'Export' not in str(deal_name):
                    deal_data = {
                        'name': str(deal_name),
                        'type': str(ws.cell(row, 8).value or ''),  # Investment Type
                        'commitment': float(ws.cell(row, 11).value or 0),  # Commitment
                        'monthly_cfs': {}  # Will store monthly cashflows
                    }

                    # Extract monthly columns (starting around column 14)
                    for col in range(14, min(14 + 60, ws.max_column + 1)):
                        month_val = ws.cell(row, col).value
                        if month_val and isinstance(month_val, (int, float)):
...
def upload_liquidity_file(contents, filename):
    """Parse uploaded Liquidity Pull Excel file"""
    if contents is None:
        return None, ""
    ...
    try:
        ...
        if fund_nav and isinstance(fund_nav, (int, float)):
            liquidity_data['fund_nav'] = float(fund_nav) / 1_000_000  # Convert to millions

        # Current Cash (row 13, column 3)
        cash = ws.cell(13, 3).value
        if cash and isinstance(cash, (int, float)):
            liquidity_data['current_cash'] = float(cash) / 1_000_000

        # GLF Balance (row 14, column 2)
        glf = ws.cell(14, 2).value
        if glf and isinstance(glf, (int, float)):
            liquidity_data['glf_balance'] = float(glf) / 1_000_000

        # CQS Balance (row 15, column 2)
        cqs = ws.cell(15, 2).value
        ...
        # Extract subscription/redemption data
        for col in range(1, 13):  # 12 months
            try:
...
def update_analytics_summary(view, deals, pipeline, placeholders):
    """Update summary cards based on view toggle"""

    if view == 'current':
        # Current portfolio only
        portfolio_deals = deals if deals else []
    else:
        # Pro forma: current + pipeline + placeholders
        portfolio_deals = (deals if deals else []) + \
                          [{'name': p['name'], 'size': p['size'], 'strategy': p.get('type', 'GP-Led'),
                            'target_irr': p.get('target_irr', 0.20)} for p in (pipeline if pipeline else [])] + \
                          [{'name': p['name'], 'size': p['size'], 'strategy': p.get('strategy', 'GP-Led'),
                            'target_irr': 0.22} for p in (placeholders if placeholders else [])]
    ...
    if view == 'current':
        ...
        portfolio_deals = (deals if deals else []) + \
                          [{'name': p['name'], 'size': p['size'],
                            'strategy': p.get('type', p.get('strategy', 'GP-Led'))}
...
if __name__ == '__main__':
    ...
    print("   • Analytics with Current/Pro Forma Toggle")
    print("   • Excel File Uploads")
